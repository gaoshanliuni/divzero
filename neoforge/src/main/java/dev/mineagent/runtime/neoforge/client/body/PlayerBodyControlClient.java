...
public final class PlayerBodyControlClient {
    ...
    static void approve(PlayerBodyReviewScreen screen,PlayerBodyPayloads.Offer offer,PlayerControlPlan plan,Object wire){
        var mc=Minecraft.getInstance();if(flight!=null||mc.screen!=screen||!screen.localConsent()||mc.getConnection()==null||mc.getConnection().getConnection()!=wire||mc.level==null||mc.player==null||!mc.player.isAlive()||!mc.isWindowActive()||!mc.level.dimension().identifier().toString().equals(offer.dimension())){screen.reject("当前客户端状态已变化，请取消并重新发起。");return;}
        ...
        send(f,"START","");}catch(Exception failure){stop("START_SEND_FAILED");screen.reject("启动失败，未自动重试。");}
    }
    ...
    @SubscribeEvent public static void tick(ClientTickEvent.Pre e){
        ...
        try{releaseKeys();if(!f.priorAction.equals(step.action())&&Set.of("ATTACK","USE").contains(f.priorAction))cancelInteraction(f);f.priorAction=step.action();
            ...
            mc.gui.setOverlayMessage(Component.literal("AI 接管本人 · "+(frame.index()+1)+"/"+f.plan.steps().size()+" "+label(step)+" · Esc 退出接管"),false);
    ...
    private static void resetMouse(){var mc=Minecraft.getInstance();var m=(dev.mineagent.runtime.neoforge.mixin.client.WorkspaceMouseAccess)mc.mouseHandler;m.mineagent$left(false);m.mineagent$middle(false);m.mineagent$right(false);m.mineagent$activeButton(null);m.mineagent$fakeRight(0);mc.mouseHandler.setIgnoreFirstMove();}
    public static boolean physicalKey(long window,int action,KeyEvent event){var mc=Minecraft.getInstance();var result=PlayerControlInputPolicy.key(flight!=null,window==mc.getWindow().handle(),event.key(),action);if(result==PlayerControlInputPolicy.Result.STOP)stop("USER_ESCAPE");return result!=PlayerControlInputPolicy.Result.PASS;}
    @SubscribeEvent(priority=net.neoforged.bus.api.EventPriority.HIGHEST) public static void mouse(InputEvent.MouseButton.Pre e){if(flight!=null)e.setCanceled(true);}
    @SubscribeEvent(priority=net.neoforged.bus.api.EventPriority.HIGHEST) public static void scroll(InputEvent.MouseScrollingEvent e){if(flight!=null)e.setCanceled(true);}
    ...
    public static String label(PlayerControlPlan.Step step){String name=switch(step.action()){case "FORWARD"->"前进";case "BACK"->"后退";case "LEFT"->"左移";case "RIGHT"->"右移";case "JUMP"->"跳跃";case "SNEAK"->"潜行";case "SPRINT_FORWARD"->"向前疾跑";case "ATTACK"->"攻击 / 挖掘";case "USE"->"使用 / 放置";case "LOOK"->"转视角 yaw "+step.yaw()+"° / pitch "+step.pitch()+"°";case "HOTBAR"->"选择快捷栏 "+(step.slot()+1);default->"等待";};return name+" · "+step.ticks()/20.0+" 秒";}
}
