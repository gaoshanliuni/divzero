...
public final class WebGuiHostAdapter implements AutoCloseable {
    ...
    private java.util.concurrent.CompletableFuture<?> handleRemote(JsonObject message) {
        ...
        if(channel.equals("settingsAction")){
            ...
            if(kind.equals("models")){if(message.has("refresh")&&(!message.get("refresh").isJsonPrimitive()||!message.get("refresh").getAsJsonPrimitive().isBoolean()))throw new IllegalArgumentException("MODELS_ARGUMENTS");if(message.has("offset")&&(!message.get("offset").isJsonPrimitive()||!message.get("offset").getAsJsonPrimitive().isNumber()||!message.get("offset").toString().matches("0|[1-9][0-9]{0,8}")))throw new IllegalArgumentException("MODELS_ARGUMENTS");boolean refresh=message.has("refresh")&&message.get("refresh").getAsBoolean();int offset=message.has("offset")?message.get("offset").getAsInt():0;String filter=message.has("query")?text(message,"query",128):"";return dev.mineagent.runtime.neoforge.client.ProviderModelsClient.query(refresh,offset,filter).thenApply(state->Map.of("code","OBSERVED","values",Map.of("state",state)));}
