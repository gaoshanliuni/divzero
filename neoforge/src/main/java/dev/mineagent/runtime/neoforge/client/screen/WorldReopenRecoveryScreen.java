...
public final class WorldReopenRecoveryScreen extends Screen {
    ...
    @Override protected void init(){
        ...
        try{
            ...
            for(var plan:plans.stream().limit(rows).toList()){
                ...
                var button=addRenderableWidget(Button.builder(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("撤销计划…")),b->confirm(plan)).bounds(left+w-100,y,100,24).build());button.active=offline()&&!plan.nativeStarted()&&!plan.state().equals("CANCEL_REQUESTED")&&Path.of(plan.savePath()).toAbsolutePath().normalize().startsWith(base);
            }
            if(plans.isEmpty())addRenderableWidget(new StringWidget(left,82,w,25,Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("没有待处理的世界重开计划。")),font));
            ...
            addRenderableWidget(Button.builder(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("只读刷新")),b->rebuildWidgets()).bounds(left+w/3,height-56,w/3-4,22).build());
            var next=addRenderableWidget(Button.builder(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("下一页")),b->{offset+=rows;rebuildWidgets();}).bounds(left+2*w/3,height-56,w/3-4,22).build());next.active=plans.size()>rows;
        }catch(Exception failure){message=code(failure);addRenderableWidget(new StringWidget(left,88,w,26,Component.literal(message),font));addRenderableWidget(Button.builder(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("重新读取")),b->rebuildWidgets()).bounds(left,height-56,w,22).build());}
    ...
    private void confirm(WorldReopenPlans.Plan plan){if(!offline())return;Minecraft.getInstance().setScreen(new ConfirmScreen(answer->{
        if(answer)try{if(!offline())throw new IllegalStateException("WORLD_REOPEN_WORLD_IN_USE");WorldReopenBootstrap.requestOfflineCancel(plan,Minecraft.getInstance().getLevelSource().getBaseDir(),"LOCAL_CLIENT_OPERATOR");message=dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("已请求撤销；正常打开原世界后保存并确认结果，不自动启动。");}catch(Exception failure){message=code(failure);}
        Minecraft.getInstance().setScreen(this);
    },Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("撤销这个尚未生效的计划？")),Component.literal("operation "+plan.input().operation()+dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("。只恢复该计划前的数据包选择，不删除世界文件或卸载已装入维度；保存的世界生成数据已变化时会拒绝。"))));}
