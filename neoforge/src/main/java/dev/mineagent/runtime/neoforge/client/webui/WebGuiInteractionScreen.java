...
public final class WebGuiInteractionScreen extends Screen {
    ...
    @Override protected void init() { KeyMapping.releaseAll(); WebGuiHostAdapter.INSTANCE.interactionMode(true); if (browser() != null) browser().setFocus(true); }
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor graphics, int x, int y, float delta) {
        ...
        if (Minecraft.getInstance().level == null && browser != null && browser.isTextureReady()) {
            if(WebGuiAtlasCompositor.enabled()){WebGuiAtlasCompositor.render(graphics,RenderPipelines.GUI_TEXTURED,browser.getTextureIdentifier(),width,height);return;}
