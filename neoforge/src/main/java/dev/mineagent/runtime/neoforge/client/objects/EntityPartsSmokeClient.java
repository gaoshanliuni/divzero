...
public final class EntityPartsSmokeClient {
    ...
    @SubscribeEvent public static void tick(ClientTickEvent.Post event){if(!EntityPartsSmokeServer.enabled())return;var mc=Minecraft.getInstance();if(mc.level==null)return;if(mc.screen!=null)mc.setScreen(null);mc.options.hideGui=true;
        ...
        if(capture>0&&++ticks==(dev.mineagent.runtime.neoforge.client.cinematic.CinematicCaptureClient.enabled()?240:25)&&!writing){writing=true;int current=capture;var file=mc.gameDirectory.toPath().resolve("entity-parts-smoke/visual-"+current+".png");mc.options.hideGui=true;
        ...
        if(EntityPartsSmokeServer.done&&!writing)dev.mineagent.runtime.neoforge.client.cinematic.CinematicCaptureClient.finish();
    }
}
