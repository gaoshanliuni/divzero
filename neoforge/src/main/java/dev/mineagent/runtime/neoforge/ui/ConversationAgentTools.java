...
public final class ConversationAgentTools {
    ...
    private static CompletableFuture<Map<String,Object>> mutate(ServerPlayer p,UUID agent,UUID operation,String tool,JsonNode a,BooleanSupplier permit)throws Exception{
        ...
        switch(tool){
            ...
            case "build_agent_path"->{keys(a,"from","to","item");return NativeAgentPathBuilder.start(p,agent,operation,a,permit);}
            case "agent_block_action"->{keys(a,"action","position","item","approach");return NativeAgentBlockActions.start(p,agent,operation,a,permit);}
