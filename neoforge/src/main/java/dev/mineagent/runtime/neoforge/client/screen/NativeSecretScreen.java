...
public final class NativeSecretScreen extends Screen {
    ...
    public NativeSecretScreen(Screen parent,String secretKey){super(Component.literal(secretKey.equals("provider.asr.apiKey")?dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("ASR 独立 Key · 原生保密输入"):dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("Provider API Key · 原生保密输入")));if(!NativeSecretPayload.KEYS.contains(secretKey))throw new IllegalArgumentException("SECRET_KEY_INVALID");this.secretKey=secretKey;this.parent=parent;connection=Minecraft.getInstance().getConnection();}
    @Override protected void init(){
        ...
        addRenderableWidget(new StringWidget(left,55,w,18,Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("密钥仅在此原生输入；网页不会接收输入值。")),font));
        ...
        addRenderableWidget(Button.builder(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("刷新版本（保留未提交输入）")),b->requestSnapshot()).bounds(left,207,w,22).build());
        addRenderableWidget(Button.builder(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("返回设置")),b->onClose()).bounds(left,239,w,22).build());
    ...
    private void submit(String value){
        ...
        try{
            ...
            ClientPacketDistributor.sendToServer(new MineAgentPayloads.SecretConfigWrite(id,expectedRevision,world,instance,Map.of("ephemeral",encoder.encodeToString(envelope.ephemeralPublicKey()),"nonce",encoder.encodeToString(envelope.nonce()),"ciphertext",encoder.encodeToString(envelope.ciphertext()))));input.setValue("");status.setMessage(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("正在加密保存…")));refreshButtons();
        }catch(Exception failure){pending=null;input.setValue("");status.setMessage(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("密钥处理失败，请刷新后重试；未回显输入。")));refreshButtons();}
    }
    public static void accept(MineAgentPayloads.SecretConfigResult result){var mc=Minecraft.getInstance();if(mc.screen instanceof NativeSecretScreen screen&&screen.current()&&result.operation().equals(screen.pending)){screen.pending=null;screen.expectedRevision=result.revision();screen.clearArmed=false;screen.input.setValue("");screen.status.setMessage(Component.literal(result.accepted()?(result.configured()?dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("密钥已保存；没有调用模型。"):dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("密钥已清除；没有调用模型。")):(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("未保存：")+result.code()+dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("。请刷新版本再操作。"))));screen.clear.setMessage(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("清除已保存 Key…")));screen.waitingSnapshot=!result.accepted();screen.refreshButtons();if(result.accepted()){screen.onClose();if(result.configured()&&screen.secretKey.equals("provider.openai.apiKey")&&!(screen.parent instanceof dev.mineagent.runtime.neoforge.client.webui.WebGuiInteractionScreen))mc.setScreen(new ProviderModelScreen(screen.parent));}}}
    @Override public void tick(){super.tick();ticks++;if(!current()){input.setValue("");status.setMessage(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("连接已改变，已清除输入。")));refreshButtons();return;}
        if(waitingSnapshot&&pending==null&&PanelSnapshotInbox.generation()!=snapshotGeneration){var snapshot=PanelSnapshotInbox.snapshot();var values=snapshot.values();try{world=UUID.fromString(values.get("security.worldId"));instance=UUID.fromString(values.get("security.configInstance"));publicKey=values.getOrDefault("security.secretTransportPublicKey","");expectedRevision=snapshot.revision();String address=values.getOrDefault(secretKey.equals("provider.asr.apiKey")?"provider.asr.baseUrl":"provider.openai.baseUrl","");endpoint.setMessage(Component.literal(dev.mineagent.runtime.core.config.WebSettingsCatalog.safeUrl(address)?address:dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("地址含私密参数，已隐藏")));waitingSnapshot=false;status.setMessage(Component.literal(Boolean.parseBoolean(values.getOrDefault("permission.manage_providers","false"))?(values.containsKey(secretKey)?dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("已配置。留空不改变；替换或清除需要明确操作。"):dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("尚未配置 API Key。保存后自动获取模型列表，不生成内容。")) :dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("没有管理 Provider 的权限。")));}catch(Exception unavailable){status.setMessage(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("服务端密钥上下文未就绪。")));}snapshotGeneration=PanelSnapshotInbox.generation();refreshButtons();}
        if(pending!=null&&ticks-waitingAt>600){pending=null;waitingSnapshot=true;input.setValue("");status.setMessage(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("结果未知。先刷新查看状态，不自动重发。")));refreshButtons();}
    }
