...
public final class ClientHostCommands implements LocalHostCommands.Endpoint {
    ...
    public Map<String,Object> inspect(){return Map.of("available",PWSH!=null,"platform",System.getProperty("os.name","unknown"),"shell","PowerShell 7","localApprovalRequired",true,"scriptMaxChars",8192,"timeoutMaxSeconds",60,"outputPreviewBytesPerStream",LocalPowerShellExecutor.OUTPUT_BYTES,"workingDirectory","current Minecraft instance/mineagent-host/workspace","notFilesystemSandbox",true,"remoteServerAccess",false);}
    ...
    public CompletableFuture<Map<String,Object>> output(MinecraftServer s,UUID id,UUID operation,String stream,int offset){return CompletableFuture.supplyAsync(()->{try{var scope=outputs.get(operation);if(!matches(s,id)||scope==null||scope.server()!=s||!scope.owner().equals(id))throw new SecurityException("HOST_OUTPUT_SCOPE");var result=new LocalPowerShellExecutor(Minecraft.getInstance().gameDirectory.toPath(),PWSH).output(operation,stream,offset);if(!matches(s,id))throw new SecurityException("HOST_OUTPUT_CONTEXT_CHANGED");return result;}catch(Exception e){return Map.<String,Object>of("status","REJECTED","error",Objects.toString(e.getMessage(),"HOST_OUTPUT_UNAVAILABLE"));}},IO);}
