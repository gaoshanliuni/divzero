...
public final class ClientHostCommands implements LocalHostCommands.Endpoint {
    ...
    public CompletableFuture<Map<String,Object>> request(MinecraftServer s,UUID id,HostCommandRequest r){var result=new CompletableFuture<Map<String,Object>>();Minecraft.getInstance().execute(()->{var mc=Minecraft.getInstance();if(!matches(s,id)||mc.getSingleplayerServer()!=s||mc.player==null||!id.equals(mc.player.getUUID())||PWSH==null){result.complete(Map.of("status","REJECTED","error","HOST_UNAVAILABLE_OR_CONTEXT_CHANGED"));return;}if(mc.screen!=null&&!(mc.screen instanceof dev.mineagent.runtime.neoforge.client.webui.WebGuiInteractionScreen)&&!(mc.screen instanceof dev.mineagent.runtime.neoforge.client.screen.ControlCenterScreen)){result.complete(Map.of("status","REJECTED","error","HOST_LOCAL_UI_BUSY"));return;}if(pending!=null){result.complete(Map.of("status","REJECTED","error","HOST_COMMAND_BUSY"));return;}var p=new Pending(r,s,id,mc.getConnection());pending=p;p.result.whenComplete((v,e)->{if(e!=null)result.complete(Map.of("status","UNKNOWN","error","HOST_RESULT_UNAVAILABLE"));else result.complete(v);});mc.setScreen(new HostCommandScreen(mc.screen,p));});return result;}
