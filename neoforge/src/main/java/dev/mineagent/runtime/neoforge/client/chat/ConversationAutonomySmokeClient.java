...
final class ConversationAutonomySmokeClient {
    ...
    static void tick()throws Exception{if(done)return;var mc=Minecraft.getInstance();ticks++;try{if(!ConversationAutonomySmokeServer.failure.isEmpty())throw new IllegalStateException(ConversationAutonomySmokeServer.failure);if(ticks>19000)throw new IllegalStateException("AUTONOMY_CLIENT_TIMEOUT");if(mc.player==null||!ConversationAutonomySmokeServer.ready)return;
        if(!sent){sent=true;mc.player.connection.sendChat("@工具助手 接管我的身体，走到附近金块地面上，遇到新障碍就绕开。到达后把我的视角转向旁边石英方块，保持接管待命，直到我按Esc主动停止。不要用命令或传送，不要每次规划都再让我确认。");}
        if(mc.screen instanceof AutonomousBodyClient.StartScreen screen&&!approved){if(!focused){focused=true;org.lwjgl.glfw.GLFW.glfwFocusWindow(mc.getWindow().handle());}var button=screen.children().stream().filter(w->w instanceof Button b&&b.getMessage().getString().equals("确认，直到我主动停止")).map(w->(Button)w).findFirst().orElse(null);if(button!=null&&button.active){approved=true;button.onPress(new KeyEvent(257,0,0));}}
