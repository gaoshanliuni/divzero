...
public final class UiAgentClient {
    ...
    public static void stop(String view,boolean notify,String signal){
        ...
        if(session!=null&&session.binding().actorKind()==ActorKind.AGENT)WebGuiHostAdapter.INSTANCE.emit("uiAgentStopped",Map.of("viewId",view,"status",dev.mineagent.runtime.api.ui.ContainerProtocol.bound(session.binding())?dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("Agent 容器任务已停止；光标物品按 Agent 自身原生规则归还，不转为玩家权限。"):dev.mineagent.runtime.api.ui.UiInteractionScope.pageOnly(session.binding())?dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("页面委派已停止，当前本地页面仍保留；重新打开新页面后才能再次委派，未授予世界写权限。"):dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("已停止：保留草稿；此文档不可写，重新打开玩家视图可继续。")));
    }
