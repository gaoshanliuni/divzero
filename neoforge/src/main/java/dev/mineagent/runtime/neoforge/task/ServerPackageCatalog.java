...
public final class ServerPackageCatalog {
    ...
    public static Map<String,String> read(ServerPlayer viewer,Map<String,String> args)throws Exception{
        ...
        if(kind.equals("creatures")){keys(args,Set.of("kind","offset"));result=dev.mineagent.runtime.neoforge.content.RuntimeCreatures.catalog(viewer,number(args,"offset"));}
        else if(kind.equals("creaturePreview")||kind.equals("modelPreview")){var data=JSON.createObjectNode();if(kind.equals("creaturePreview")){keys(args,Set.of("kind","speciesId","revision"));data.put("kind","creature").put("species_id",args.get("speciesId")).put("expected_revision",Long.parseLong(args.get("revision")));}else{keys(args,Set.of("kind","packageId","definitionId","path","revision"));data.put("kind","package_model").put("package_id",args.get("packageId")).put("definition_id",args.get("definitionId")).put("model_path",args.get("path")).put("expected_revision",Long.parseLong(args.get("revision")));}dev.mineagent.runtime.neoforge.ui.ServerPreviews.open(viewer,null,data,()->server.getPlayerList().getPlayer(viewer.getUUID())==viewer).whenComplete((r,e)->{if(e!=null)server.execute(()->{if(server.getPlayerList().getPlayer(viewer.getUUID())==viewer)viewer.sendSystemMessage(net.minecraft.network.chat.Component.literal("预览未打开：模型不存在、格式不支持或版本已改变。"));});});result=Map.of("status","PREPARING_PREVIEW");}
        else if(kind.equals("packages")){
            ...
            else if(kind.equals("models")){var models=new ArrayList<Object>();for(var d:head.definitions().values())for(String path:d.resourcePaths()){var ref=head.resources().get(path);if(ref!=null&&ref.mediaType().equals("application/json")&&ref.side()!=dev.mineagent.runtime.api.packages.RuntimeResourceSide.SERVER)models.add(Map.of("definitionId",d.definitionId(),"name",d.name(),"path",path));}int offset=number(args,"offset");result=Map.of("items",models.stream().skip(offset).limit(16).toList(),"more",offset+16<models.size(),"nextOffset",offset+16);}
