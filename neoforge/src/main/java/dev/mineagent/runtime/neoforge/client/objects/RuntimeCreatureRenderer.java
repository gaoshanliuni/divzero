...
public final class RuntimeCreatureRenderer extends EntityRenderer<RuntimeCreatureEntity,RuntimeCreatureRenderer.State> {
    ...
    public static final class State extends EntityRenderState{BorrowedCreatureVisuals.View borrowed;RuntimeMesh mesh;UUID entity;float yaw;boolean baby;String animationEvent="idle";List<dev.mineagent.runtime.core.creature.CreatureRig.Bone> bones=List.of();dev.mineagent.runtime.core.creature.CreatureAnimation.Clip clip;double animationAge;double[] transform={0,0,0,0,0,0,1,1,1};}
    ...
    @Override public void submit(State state,PoseStack poses,SubmitNodeCollector collector,CameraRenderState camera){
        ...
        if(state.bones.isEmpty())drawMesh(state.mesh,state,poses,collector);else {
            var rootInverse=CreatureRigTelemetry.enabled()?new org.joml.Matrix4f(poses.last().pose()).invert():null;
            ...
            for(var bone:state.bones){if(bone.mesh()==null)continue;poses.pushPose();var chain=new ArrayList<dev.mineagent.runtime.core.creature.CreatureRig.Bone>();for(var b=bone;b!=null;b=byName.get(b.parent()))chain.add(b);Collections.reverse(chain);
                ...
                if(rootInverse!=null)CreatureRigTelemetry.record(state.entity,bone.name(),state.animationEvent,rootInverse,poses.last().pose());
