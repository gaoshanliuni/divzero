...
public final class PlayerBodyReviewScreen extends Screen {
    ...
    public PlayerBodyReviewScreen(PlayerBodyPayloads.Offer offer,PlayerControlPlan plan,Object wire){super(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("AI 接管本人 · 审核完整操作计划")));this.offer=offer;this.plan=plan;this.wire=wire;player=Minecraft.getInstance().player;level=Minecraft.getInstance().level;}
    ...
    @Override protected void init(){int left=Math.max(8,width/2-260),w=Math.min(520,width-16),rows=Math.max(1,(height-155)/15);page=Math.max(0,Math.min(page,(plan.steps().size()-1)/rows));
        addRenderableWidget(new StringWidget(left,10,w,18,getTitle(),font));addRenderableWidget(new StringWidget(left,30,w,16,Component.literal(offer.agent()+dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t(" · 控制你当前真实角色，不是独立 AI 身体")),font));
        ...
        if(plan.steps().size()>rows){addRenderableWidget(Button.builder(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("上一页")),b->{page=Math.max(0,page-1);rebuildWidgets();}).bounds(left,height-97,75,20).build());addRenderableWidget(Button.builder(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("下一页")),b->{page=Math.min((plan.steps().size()-1)/rows,page+1);rebuildWidgets();}).bounds(left+80,height-97,75,20).build());}
        addRenderableWidget(new StringWidget(left,height-74,w,14,Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("仅 Esc 手动退出；其它游戏键鼠忽略。失焦、死亡、断线自动释放。")),font));
        status=addRenderableWidget(new StringWidget(left,height-56,w,14,Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("会发生真实移动和交互；不保证寻路成功。请先逐项核对。")),font));
        start=addRenderableWidget(Button.builder(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("确认并开始接管本人")),b->{requested=true;quiet=0;status.setMessage(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("请松开键鼠；等待本机释放输入和服务器批准…")));}).bounds(left,height-33,w/2-4,22).build());
        var cancel=addRenderableWidget(Button.builder(Component.literal(dev.mineagent.runtime.neoforge.client.language.ClientLanguage.t("取消，不接管")),b->onClose()).bounds(left+w/2+4,height-33,w/2-4,22).build());setInitialFocus(cancel);
    }
