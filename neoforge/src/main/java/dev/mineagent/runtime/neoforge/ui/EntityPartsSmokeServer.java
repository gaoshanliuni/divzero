...
public final class EntityPartsSmokeServer {
    ...
    @SubscribeEvent public static void tick(ServerTickEvent.Post event){if(!enabled()||done)return;var server=event.getServer();player=server.getPlayerList().getPlayers().stream().filter(p->!(p instanceof MineAgentPlayer)).findFirst().orElse(null);if(player==null)return;
        try{
            ...
            if(sheep!=null){sheep.setYRot(0);sheep.yBodyRot=0;sheep.setYHeadRot(phase.equals("ROTATED")?35:0);sheep.setXRot(phase.equals("ROTATED")?20:0);}
            ...
            if(phase.equals("VERIFY")&&pending.isDone()){
                ...
                long sheepCount=0,witherCount=0;for(var e:player.level().getAllEntities()){if(e.getType()==EntityType.SHEEP)sheepCount++;if(e.getType()==EntityType.WITHER)witherCount++;}check(sheepCount==2&&witherCount==0,"DONOR_SPAWN_OR_SHEEP_RECREATED");check(sheep.getUUID().equals(sheepId)&&sheep.getType()==EntityType.SHEEP&&sheep.getHealth()==8&&sheep.isNoAi()&&!sheep.isSheared(),"SHEEP_GAMEPLAY_CHANGED");
            ...
            if(phase.equals("ROTATED_PROBE")&&pending.isDone()){save("rotated-draw",probe(pending.join()));pending=call("delete_entity_rule",JSON.createObjectNode().put("rule_id",rule.toString()).put("expected_revision",revision));phase="REMOVE";return;}
