...
public final class AgentSkinClient {
    ...
    public static PlayerSkin skin(UUID id){var mc=Minecraft.getInstance();if(mc.getConnection()==null||mc.getConnection().getConnection()!=wire){skins=Map.of();wire=null;return null;}var choice=skins.get(id);if(choice==null)return null;if(choice.skin().startsWith("png:")){var custom=AgentPngSkinClient.skin(id);return custom==null?net.minecraft.client.resources.DefaultPlayerSkin.get(id):custom;}if(choice.skin().equals("player")){var info=mc.getConnection().getPlayerInfo(choice.player());return info==null?net.minecraft.client.resources.DefaultPlayerSkin.get(choice.player()):info.getSkin();}var parts=choice.skin().split(":");return new PlayerSkin(new ClientAsset.ResourceTexture(Identifier.withDefaultNamespace("entity/player/"+parts[1]+"/"+parts[0])),null,null,parts[1].equals("slim")?PlayerModelType.SLIM:PlayerModelType.WIDE,true);}
