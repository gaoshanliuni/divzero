...
final class ConversationThrowItemSmokeClient {
    ...
    static void tick()throws Exception{
        ...
        try{
            ...
            if(ConversationRuntimeItemSmokeServer.verified){mc.options.keyUse.setDown(false);if(RuntimeThrownItemRenderer.rendered<1)throw new IllegalStateException("THROW_CLIENT_FLIGHT_NOT_DRAWN");Files.writeString(root.resolve("client.json"),new com.google.gson.Gson().toJson(Map.of("flightRenderSubmissions",RuntimeThrownItemRenderer.rendered,"nativeUseAndRelease",true,"systemInputInjected",false,"keyMappingReleased",!mc.options.keyUse.isDown())));done=true;dev.mineagent.runtime.neoforge.MineAgentRuntimeMod.LOGGER.info("MINEAGENT_CONVERSATION_AGENT_OK");dev.mineagent.runtime.neoforge.client.cinematic.CinematicCaptureClient.finish();}
        }catch(Exception failure){mc.options.keyUse.setDown(false);done=true;Files.writeString(root.resolve("client-failure.json"),new com.google.gson.Gson().toJson(Map.of("error",failure.toString(),"request",seen,"holding",holding)));dev.mineagent.runtime.neoforge.client.cinematic.CinematicCaptureClient.finish();}
    }
}
