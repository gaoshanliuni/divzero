...
final class ConversationThrowItemSmokeClient {
    private static int ticks,wait,seen,holding;private static boolean sent,done;private static boolean captured;
    private static void capture(Path path){net.minecraft.client.Screenshot.takeScreenshot(Minecraft.getInstance().getMainRenderTarget(),image->{try(image){image.writeToFile(path);}catch(Exception e){ConversationRuntimeItemSmokeServer.failure=e.toString();}});}
