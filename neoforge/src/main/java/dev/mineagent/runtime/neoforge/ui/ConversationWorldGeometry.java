...
public final class ConversationWorldGeometry {
    ...
    private static final class Plan {
        ...
        final Map<String,Long> desiredStates=new TreeMap<>();final Map<String,BlockState> parsed=new HashMap<>();
        String status="SAMPLING",error="";long selected,skipped,unchanged,written,matched,mismatched;
    ...
    private static BlockState parse(ServerPlayer p,String value)throws Exception{
        var reader=new com.mojang.brigadier.StringReader(value);BlockStateParser.BlockResult result;try{result=BlockStateParser.parseForBlock(p.registryAccess().lookupOrThrow(Registries.BLOCK),reader,false);}catch(com.mojang.brigadier.exceptions.CommandSyntaxException invalid){throw new IllegalArgumentException("GEOMETRY_BLOCK_STATE_INVALID",invalid);}require(!reader.canRead(),"BLOCK_STATE_TRAILING_DATA");require(!result.blockState().hasBlockEntity(),"BLOCK_ENTITY_NOT_SUPPORTED");return result.blockState();
    }
    private static BlockState cached(ServerPlayer p,Plan plan,String value)throws Exception{var state=plan.parsed.get(value);if(state==null){state=parse(p,value);plan.parsed.put(value,state);}return state;}
    private static BlockState oriented(BlockState b,List<String> ops){for(String op:ops)b=switch(op){case "r1"->b.rotate(Rotation.CLOCKWISE_90);case "r2"->b.rotate(Rotation.CLOCKWISE_180);case "r3"->b.rotate(Rotation.COUNTERCLOCKWISE_90);case "mx"->b.mirror(Mirror.FRONT_BACK);case "mz"->b.mirror(Mirror.LEFT_RIGHT);default->throw new IllegalArgumentException("GEOMETRY_ORIENTATION");};return b;}
    ...
    private static Map<String,Object> summary(Plan p){
        var out=new LinkedHashMap<String,Object>();out.put("planId",p.id);out.put("status",p.status);out.put("error",p.error);out.put("expiresAt",p.expires);out.put("origin",p.geometry.origin().list());out.put("parts",p.geometry.parts());out.put("generated",p.generated);out.put("selected",p.selected);out.put("maskSkipped",p.skipped);out.put("unchanged",p.unchanged);out.put("written",p.written);out.put("matchedAfterPhysics",p.matched);out.put("mismatchedAfterPhysics",p.mismatched);out.put("desiredStates",Map.copyOf(p.desiredStates));out.put("bounds",Map.of("min",p.geometry.min().list(),"max",p.geometry.max().list()));return out;
    }
    ...
    private static void sample(ServerPlayer p,UUID agent,State st,Plan plan,BooleanSupplier permit,CompletableFuture<Map<String,Object>> result){
        var level=plan.level;st.jobs.add(new Work(){CompletableFuture<List<dev.mineagent.runtime.core.geometry.GeometrySpool.Row>> pending=io(()->plan.spool.page(0,BATCH,false));
            ...
            }catch(Exception e){st.planning.remove(p.getUUID());result.complete(Map.of("status","REJECTED","error",code(e)));dispose(plan);return true;}}
    ...
    public static CompletableFuture<Map<String,Object>> apply(ServerPlayer p,UUID agent,JsonNode a,BooleanSupplier permit){
        ...
        state(level.getServer()).jobs.add(new Work(){int phase;long verifyTick;CompletableFuture<List<dev.mineagent.runtime.core.geometry.GeometrySpool.Row>> pending=io(()->plan.spool.page(0,BATCH,true));
            ...
            public boolean tick(){try{
                require(current(p,agent,level,plan.permission,permit),"CONTEXT_CHANGED");if(phase==2&&level.getGameTime()<verifyTick||!pending.isDone())return false;var rows=pending.join();
                if(rows.isEmpty()){if(phase==2){plan.expires=System.currentTimeMillis()+600000;plan.status=plan.mismatched==0?"APPLIED":"PARTIAL";if(plan.mismatched>0)plan.error="GEOMETRY_PHYSICS_CHANGED_STATE";result.complete(summary(plan));return true;}phase++;if(phase==2)verifyTick=level.getGameTime()+4;pending=io(()->plan.spool.page(0,BATCH,true));return false;}
                for(var r:rows){BlockPos pos=new BlockPos(r.x(),r.y(),r.z());location(level,pos);BlockState before=cached(p,plan,r.before()),after=cached(p,plan,r.after());
                    if(phase<2){require(level.getBlockState(pos)==before,"STATE_CHANGED");if(before!=after)writable(level,pos,after);if(phase==1&&before!=after){boolean changed=level.setBlock(pos,after,Block.UPDATE_ALL);if(level.getBlockState(pos)!=before)plan.written++;require(changed,"WRITE_REJECTED");}}
                    else if(level.getBlockState(pos)==after)plan.matched++;else plan.mismatched++;
                }
