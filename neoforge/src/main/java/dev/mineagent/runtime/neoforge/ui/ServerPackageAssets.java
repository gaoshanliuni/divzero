...
public final class ServerPackageAssets {
    ...
    public static RuntimePackage source(ServerPlayer viewer,Input input)throws Exception {
        ...
        if(input.action().equals("COPY_VERSION")){var runtime=ServerPackageRuntime.get(viewer.level().getServer());runtime.ownedPackage(viewer.getUUID(),input.source(),input.expectedRevision()).orElseThrow(()->new SecurityException("PACKAGE_ASSET_NOT_OWNED"));var version=runtime.worldLibrary().versionByCanonical(input.source(),input.sourceRevision(),input.sourceHash());if(version.activationMode()!=dev.mineagent.runtime.api.packages.ActivationMode.HOT_RUNTIME)throw new IllegalArgumentException("PACKAGE_HISTORY_HOT_ONLY");return version;}
    ...
    public static Map<String,Object> read(ServerPlayer viewer,Map<String,String> args)throws Exception {
        ...
        if(kind.equals("receipt")){if(!args.keySet().equals(Set.of("kind","operationId")))throw new IllegalArgumentException("PACKAGE_ASSET_ARGUMENTS");var receipt=library.assetReceipt(MineAgentRuntimeServices.worldId(viewer.level().getServer()),viewer.getUUID(),UUID.fromString(args.get("operationId")));if(receipt==null)throw new IllegalStateException("PACKAGE_ASSET_RECEIPT_MISSING");if(receipt.target()!=null&&Set.of("COPY","COPY_VERSION","REUSE_ASSET").contains(receipt.input().action()))library.refreshAsset(receipt.target());return Map.of("receipt",receipt);}
