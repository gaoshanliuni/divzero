...
final class ConversationDiamondSmokeClient {
    ...
    static void tick()throws Exception{
        ...
        try{if(!ConversationDiamondSmokeServer.failure.isEmpty())throw new IllegalStateException(ConversationDiamondSmokeServer.failure);if(ticks>14000)throw new IllegalStateException("DIAMOND_TIMEOUT");if(mc.player==null||!ConversationDiamondSmokeServer.ready)return;
            ...
            if(ConversationDiamondSmokeServer.verified){done=true;dev.mineagent.runtime.neoforge.MineAgentRuntimeMod.LOGGER.info("MINEAGENT_CONVERSATION_AGENT_OK");dev.mineagent.runtime.neoforge.client.cinematic.CinematicCaptureClient.finish();}
        }catch(Exception e){done=true;Files.writeString(Files.createDirectories(mc.gameDirectory.toPath().resolve("diamond-smoke")).resolve("client-failure.txt"),e.toString());dev.mineagent.runtime.neoforge.client.cinematic.CinematicCaptureClient.finish();}
    }
}
