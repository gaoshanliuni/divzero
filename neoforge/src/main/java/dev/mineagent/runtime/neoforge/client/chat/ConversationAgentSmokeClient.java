...
public final class ConversationAgentSmokeClient {
    ...
    @SubscribeEvent public static void tick(ClientTickEvent.Post e)throws Exception{if(!Boolean.getBoolean("mineagent.conversationAgentSmoke")||finished)return;if(dev.mineagent.runtime.neoforge.ui.ConversationInteractionSmokeServer.active()){ConversationInteractionSmokeClient.tick();return;}if(dev.mineagent.runtime.neoforge.ui.ConversationBuildSmokeServer.active()){ConversationBuildSmokeClient.tick();return;}var mc=Minecraft.getInstance();ticks++;
