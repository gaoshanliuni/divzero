...
public final class ServerConversations implements AutoCloseable {
    ...
    private static final class Flight {final UUID op;final ServerPlayer viewer;final Object level;final java.util.List<java.util.Map<String,Object>> tools=new java.util.ArrayList<>();final StringBuilder reply=new StringBuilder();int rounds,calls;dev.mineagent.runtime.core.memory.PlayerPreferenceStore.Snapshot preferences;final AtomicBoolean permit=new AtomicBoolean(true);final StringBuilder buffer=new StringBuilder();volatile String error="";UUID summaryJob;int summaryCalls;Flight(UUID op,ServerPlayer viewer){this.op=op;this.viewer=viewer;this.level=viewer.level();}}
    ...
    public void submitNative(ServerPlayer viewer,UUID agent,String text,boolean explicitMention,UUID op)throws Exception{
        thread();if(!MineAgentRuntimeServices.permissions(server).allowed(viewer.getUUID(),viewer.permissions().hasPermission(net.minecraft.server.permissions.Permissions.COMMANDS_GAMEMASTER),dev.mineagent.runtime.api.permission.PermissionAction.CHAT))throw new SecurityException("CONVERSATION_FORBIDDEN");
    ...
    private Map<String,String> write(ServerPlayer viewer,UUID operation,Map<String,String> args,boolean nativeChat)throws Exception{
        ...
        if(kind.equals("route")){var f=focused(viewer).filter(v->v.conversationId().equals(id)&&v.agentId().equals(agent)&&v.contextId().toString().equals(args.get("contextId"))).orElseThrow(()->new IllegalStateException("STALE_CONVERSATION_FOCUS"));boolean enabled="true".equals(args.get("enabled"));if(enabled&&!store.get(viewer.getUUID(),agent,id).state().equals("ACTIVE"))throw new IllegalStateException("CONVERSATION_READ_ONLY");focus.nativeInput(viewer.getUUID(),viewer,f.contextId(),enabled);return value(focusedValue(f));}
        ...
        if(Set.of("rename","archive","delete","restore").contains(kind))return value(store.change(viewer.getUUID(),agent,id,operation,Long.parseLong(args.get("expectedRevision")),kind,args.getOrDefault("title","")));
        if(kind.equals("cancel")){UUID target=UUID.fromString(args.get("targetOperation"));var state=store.cancel(viewer.getUUID(),agent,id,operation,target);var flight=flights.get(target);if(flight!=null){flight.permit.set(false);if(flight.summaryJob!=null)summaries.fail(flight.summaryJob,"CANCELLED","USER_CANCELLED");}return value(state);}
        ...
        try{
            ...
            var preferences=MineAgentRuntimeServices.preferences(server).snapshot(viewer.getUUID(),MineAgentRuntimeServices.worldId(server),agent,"CONVERSATION");var plan=ConversationContext.build(store,viewer.getUUID(),definition,id,turn,persona,original,budget,null,preferences.section());var flight=new Flight(operation,viewer);flight.preferences=preferences;flights.put(operation,flight);
    ...
    private void advance(Flight flight,Generation g,ConversationSummaryStore.Summary previous)throws Exception{
        ...
        flight.summaryJob=job.summaryId();var settings=MineAgentRuntimeServices.config(server).snapshot();String model=String.valueOf(dev.mineagent.runtime.core.config.WebSettingsCatalog.routing(settings).get("textModel"));
        ...
        MineAgentRuntimeServices.worker(server).summarizeConversation(MineAgentRuntimeServices.config(server),job.summaryId(),job.prompt(),flight.permit::get).whenComplete((response,error)->server.execute(()->{
    ...
    private void failGeneration(Flight f,String code){try{store.finish(f.op,"FAILED",null,code);}catch(Exception ignored){}retire(f);}
    private boolean live(Flight f){return !closed&&f.permit.get()&&server.getPlayerList().getPlayer(f.viewer.getUUID())==f.viewer&&f.viewer.level()==f.level&&(f.preferences==null||MineAgentRuntimeServices.preferences(server).current(f.preferences));}
    ...
    private void agentRound(Flight f,Generation g,ConversationContext.Plan plan)throws Exception{
        if(!live(f)||!store.pending(f.op)){failGeneration(f,"CONVERSATION_CONTEXT_CHANGED");return;}
        if(f.rounds>=ConversationTools.MAX_ROUNDS||f.calls>ConversationTools.MAX_TOOL_CALLS){failGeneration(f,"CONVERSATION_TOOL_BUDGET");return;}
        String prompt=ConversationTools.instructions()+plan.prompt();int estimate=prompt.getBytes(java.nio.charset.StandardCharsets.UTF_8).length+json.writeValueAsBytes(f.tools).length+json.writeValueAsBytes(ConversationTools.ALL).length;
        ...
        MineAgentRuntimeServices.worker(server).streamConversation(MineAgentRuntimeServices.config(server),"SEMANTIC",prompt,delta->{if(!f.permit.get())return;String text=String.valueOf(delta.payload().getOrDefault("delta",""));synchronized(f.buffer){if(f.reply.length()+text.length()>131072){f.error="CONVERSATION_RESPONSE_TOO_LARGE";f.permit.set(false);}else{f.buffer.append(text);f.reply.append(text);}}},f.permit::get,g.preferences(),request,List.copyOf(f.tools)).whenComplete((response,error)->server.execute(()->{
            if(closed)return;try{
                flush(f);if(!store.pending(f.op)){retire(f);return;}if(!f.error.isEmpty())throw new IllegalStateException(f.error);if(!live(f))throw new IllegalStateException("CONVERSATION_CONTEXT_CHANGED");
                if(error!=null||response==null||!response.type().equals("model.stream.result")){for(Throwable cause=error;cause!=null;cause=cause.getCause())if(cause instanceof java.util.concurrent.TimeoutException)throw new IllegalStateException("CONVERSATION_STREAM_TIMEOUT");String reason=response==null?"":String.valueOf(response.payload().getOrDefault("message",""));throw new IllegalStateException(reason.matches("(?:TOOL_STREAM|STREAM)_[A-Z_]{1,60}")?"CONVERSATION_"+reason:"CONVERSATION_MODEL_FAILED");}
                ...
                if(f.calls+raw.size()>ConversationTools.MAX_TOOL_CALLS)throw new IllegalStateException("CONVERSATION_TOOL_BUDGET");f.calls+=raw.size();var calls=new ArrayList<com.fasterxml.jackson.databind.JsonNode>();var seen=new HashSet<String>();var assistantCalls=new ArrayList<Object>();
