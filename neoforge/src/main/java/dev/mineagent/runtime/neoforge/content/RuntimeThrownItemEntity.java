...
public final class RuntimeThrownItemEntity extends Entity {
    ...
    public String throwerName(){return throwerName;}
    private RuntimeItemBinding binding;private RuntimeMesh mesh;private UUID thrower;private String transfer="PREPARING";
    private final java.util.List<TrajectoryTriggers.Point> motionPath=new java.util.ArrayList<>();private int motionTick=-1;
    public int motionTick(){return motionTick;}
    public boolean crossedDownwardCircle(double x,double y,double z,double radius){return mesh!=null&&!level().isClientSide()&&motionTick==tickCount&&TrajectoryTriggers.downwardCircle(motionPath,x,y,z,radius,mesh.collision());}
    ...
    void prepare(ItemStack stack,UUID actor,String name,int used,ObjectPhysics.Motion motion){if(stack.getCount()!=1)throw new IllegalArgumentException("ITEM_TRANSFER_COUNT");entityData.set(ITEM,stack.copy());refresh();if(binding==null||!mesh.physics().dynamic()||mesh.collision().width()>1||mesh.collision().height()>1||mesh.collision().depth()>1)throw new IllegalArgumentException("ITEM_THROW_MODEL");thrower=actor;throwerName=name;usedTicks=used;launchSpeed=Math.sqrt(motion.x()*motion.x()+motion.y()*motion.y()+motion.z()*motion.z());setDeltaMovement(motion.x(),motion.y(),motion.z());}
    ...
    @Override public void tick(){
        ...
        motionPath.clear();motionTick=tickCount;motionPath.add(new TrajectoryTriggers.Point(getX(),getY(),getZ()));
        ...
        for(int i=0;i<steps;i++){
            Vec3 wanted=new Vec3(motion.x()/steps,motion.y()/steps,motion.z()/steps),before=position();move(MoverType.SELF,wanted);var actual=position().subtract(before);motionPath.add(new TrajectoryTriggers.Point(getX(),getY(),getZ()));
