...
public final class ConversationBodyTools {
    ...
    public static Map<String,Object> execute(ServerPlayer p,UUID agent,JsonNode a,boolean read)throws Exception{
        ...
        if(!read){if(!b.canAct())throw new IllegalStateException("AGENT_BODY_NOT_ALIVE");if(b.taskControlOwned()&&!(action.equals("stop")&&NativeAgentBlockActions.stop(p,agent)))throw new IllegalStateException("AGENT_BODY_TASK_OWNS_INPUT");
