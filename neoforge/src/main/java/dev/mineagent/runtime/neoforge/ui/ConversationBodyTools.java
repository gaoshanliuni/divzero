...
public final class ConversationBodyTools {
    ...
    public static Map<String,Object> execute(ServerPlayer p,UUID agent,JsonNode a,boolean read)throws Exception{
        ...
        if(!read){if(!b.canAct())throw new IllegalStateException("AGENT_BODY_NOT_ALIVE");if(b.taskControlOwned())throw new IllegalStateException("AGENT_BODY_TASK_OWNS_INPUT");
            if(!Set.of("move","look","sprint","select","drop","follow","stop","teleport_to_player").contains(action))throw new IllegalArgumentException("AGENT_BODY_ACTION");
            ...
            switch(action){
                case "move","look"->{var n=a.path("target");if(!n.isArray()||n.size()!=3)throw new IllegalArgumentException("AGENT_BODY_TARGET");double[] v=new double[3];for(int i=0;i<3;i++){if(!n.get(i).isNumber()||!Double.isFinite(v[i]=n.get(i).asDouble())||Math.abs(v[i])>30000000)throw new IllegalArgumentException("AGENT_BODY_TARGET");}var target=new Vec3(v[0],v[1],v[2]);if(action.equals("move")){b.movementController().moveTo(target);status="STARTED_NOT_ARRIVED";}else b.lookAt(net.minecraft.commands.arguments.EntityAnchorArgument.Anchor.EYES,target);}
        ...
        return Map.of("status",status,"entity",agent,"position",List.of(b.getX(),b.getY(),b.getZ()),"yaw",b.getYRot(),"pitch",b.getXRot(),"sprinting",b.isSprinting(),"following",Boolean.parseBoolean(config.snapshot().values().getOrDefault(key,"false")),"movement",b.movementController().outcome(),"inventory",rows,"mainHand",b.getMainHandItem().toString());
    }
}
