...
public final class ConversationRuntimeItemSmokeServer {
    ...
    public static boolean saved(){return System.getProperty("mineagent.conversationAgentScenario","").equals("runtime_item_saved");}
    public static boolean active(){return Boolean.getBoolean("mineagent.conversationAgentReal")&&(saved()||System.getProperty("mineagent.conversationAgentScenario","").equals("runtime_item"));}
    ...
    public static void tick(MinecraftServer s){
        ...
        try{
            ...
            if(phase==0){
                ...
                if(!p.getMainHandItem().isEmpty()||!runtime.list(p.getUUID()).isEmpty())throw new IllegalStateException("RUNTIME_ITEM_EXECUTED_WITHOUT_APPROVAL");
            ...
            if(phase==1){
                ...
                if(binding==null||!stack.is(MineAgentRegistries.RUNTIME_ITEM.get())||stack.getCount()!=1||!runtime.itemActive(p,stack))throw new IllegalStateException("RUNTIME_ITEM_NATIVE_STACK_MISSING");firstHash=binding.assetHash();var states=runtime.instance(instance).orElseThrow().state();var keys=states.keySet().stream().filter(k->(k.equals("uses")||k.endsWith("_uses"))&&states.get(k).equals("0")).toList();if(keys.size()!=1)throw new IllegalStateException("RUNTIME_ITEM_COUNTER_NOT_DISCOVERABLE");usesKey=keys.getFirst();save(s,"before-use",Map.of("name",stack.getHoverName().getString(),"binding",binding,"count",stack.getCount(),"state",runtime.instance(instance).orElseThrow().state()));itemReady=true;phase=2;
            }
            if(phase==2){
