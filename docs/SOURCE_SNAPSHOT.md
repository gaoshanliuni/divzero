...
### Entity interoperability verification closure — 1.0.14

Actions `36098988165` passed. Twilight Forest 4.9.3722 plus its declared Beanification 1.9.132 dependency were loaded in isolated instances, never added to the production mods directory. The complete live test used 11 official high-thinking deepseek-flash requests, all completed, with no artificial max_tokens (available usage: 295228 tokens). The AI inspected a real Naga, installed attack/movement and animation rules, derived a friendly 40-health hot creature using its head renderer, spawned it, and verified its rendering.
...
Fresh native entity templates now retain the installed original entity implementation instead of borrowing only its head renderer. AI tools can inspect, derive a selected-property recipe, save it, spawn independent instances and apply changes. Twilight Forest combat-root discovery and six-member Knight Phantom initialization are included; source Mod remains required. Current scope is not arbitrary private-code cloning or complete all-boss combat acceptance. Versions 1.0.15/1.0.16 passed Actions and isolated native spawn/tick/serialization checks for two vanilla samples, all eight combat boss roots and Quest Ram. Six real DeepSeek Flash/high requests discovered the tools and created/read back a full native Naga. Its UUID, template and twelve body parts survived a same-world process restart; subsequent tests used no model calls. Hydra has 46 native parts; Snow Queen has seven; Knight Phantom initializes six numbered entities. Original Mod remains required. Full attack/phase/loot/progression combinations, arbitrary variant/NBT cloning and universal non-Mob compatibility are not claimed verified. A copied-world identity refusal and initially obscured screenshots are retained as failed evidence; unobstructed visuals were subsequently checked.


## Entity part replacement increment

New client-model discovery and `replace_entity_part` let AI attach a donor ModelPart and its texture to an existing entity’s animated part (e.g. Sheep head to Wither center head). Sheep body/wool and gameplay remain unchanged; rules can be removed to restore. Dedicated geometry submissions avoid shared-model contamination, with corresponding overlay parts hidden. Requires client/server protocol 10; arbitrary custom renderers/variants are not claimed supported. 1.0.19 passed Actions and isolated native verification. Four real DeepSeek Flash/high requests discovered the Sheep/Wither models, changed one existing sheep and read actual donor rendering. Zero-provider regression verified unshorn/sheared variants, unaffected adjacent sheep, native head transform matching (observed matrix delta 0.0), and restoration after deleting the rule. No real Wither was spawned. This verifies the adult Sheep-to-Wither-head case, not every renderer, variant or multiplayer combination.
