...
当前项目版本按用户指定设为 **1.0.1**；构建配置、JAR metadata、关于页与源码manifest使用同一版本。版本号不等于所有历史验收均已完成，本文列出的边界继续有效。
...
按最新要求统一由GitHub Actions构建main；当前Mod版本1.0.1，Tag/Release标题使用Mod版本，主JAR文件名为DivZero-mineagent-1.0.1.jar。
