...
public final class ProductionJointAppearanceLauncher {
    ...
    static void validateConversationAgentMode(Map<?,?> flags){
        ...
        if(!Set.of("","rules","cobble","blueprint","web","interaction","runtime_item","runtime_item_saved","runtime_throw","runtime_throw_saved").contains(scenario)||!real&&(!scenario.isBlank()||!budget.isBlank()))throw new IllegalArgumentException("CONVERSATION_AGENT_REAL_SCENARIO");
        ...
        String item=Objects.toString(flags.get("mineagent.conversationAgentItemArtifact"),"");if(Set.of("runtime_item_saved","runtime_throw_saved").contains(scenario)==item.isBlank())throw new IllegalArgumentException("RUNTIME_ITEM_ARTIFACT_SOURCE");
        if(!active)return;
    ...
    public static void main(String[] args)throws Exception{
        ...
        if(conversationAgentReal){
            ...
            if(Set.of("runtime_item_saved","runtime_throw_saved").contains(System.getProperty("mineagent.conversationAgentScenario",""))){Path artifact=Path.of(System.getProperty("mineagent.conversationAgentItemArtifact"));if(!Files.isRegularFile(artifact,LinkOption.NOFOLLOW_LINKS)||Files.size(artifact)>1024*1024)throw new IllegalArgumentException("RUNTIME_ITEM_ARTIFACT_INVALID");Files.copy(artifact,game.resolve("runtime-item-source.json"));}
        ...
        for(String stage:bootUpgrade?List.of("prepare","upgrade","verify","rollback"):bootDependency?List.of("prepare","verify","removed"):packageAssets?List.of("prepare","reuse","verify"):settings?personaStages(true):bodyRecovery?bodyRecoveryStages():opacityPersistence?opacityPersistenceStages():persona||conversation||audience||events||schedule||shared||sharedAgent||clientScript||clientJava?personaStages(true):stages(recovery)){
            ...
            if(conversationAgentReal){builder.environment().put("MINEAGENT_REAL_PROVIDER_AUDIT_DIR",game.resolve("real-provider-audit").toString());String cap=System.getProperty("mineagent.conversationAgentCallBudget","");builder.environment().put("MINEAGENT_REAL_PROVIDER_MAX_CALLS",Set.of("runtime_item_saved","runtime_throw_saved").contains(System.getProperty("mineagent.conversationAgentScenario",""))?"0":cap.isBlank()?"24":cap);}
