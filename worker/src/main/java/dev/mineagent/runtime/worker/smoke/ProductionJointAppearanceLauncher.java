...
public final class ProductionJointAppearanceLauncher {
    ...
    static void validateConversationAgentMode(Map<?,?> flags){
        ...
        String item=Objects.toString(flags.get("mineagent.conversationAgentItemArtifact"),"");if(Set.of("building_files_zero","building_files","python_host_saved","creature_events_saved","watch_ui_saved","creatures_saved","thinking_saved","runtime_item_saved","runtime_throw_saved","runtime_throw_sphere_saved","basketball_saved","management_ysm_saved").contains(scenario)==item.isBlank())throw new IllegalArgumentException("RUNTIME_ITEM_ARTIFACT_SOURCE");
        if(!active)return;
    ...
    public static void main(String[] args)throws Exception{
        ...
        if(conversationAgentReal){
            ...
            if(Set.of("building_files_zero","building_files","python_host_saved","creature_events_saved","watch_ui_saved","creatures_saved","thinking_saved","runtime_item_saved","runtime_throw_saved","runtime_throw_sphere_saved","basketball_saved","management_ysm_saved").contains(System.getProperty("mineagent.conversationAgentScenario",""))){Path artifact=Path.of(System.getProperty("mineagent.conversationAgentItemArtifact"));if(!Files.isRegularFile(artifact,LinkOption.NOFOLLOW_LINKS)||Files.size(artifact)>1024*1024)throw new IllegalArgumentException("RUNTIME_ITEM_ARTIFACT_INVALID");Files.copy(artifact,game.resolve("runtime-item-source.json"));}
        ...
        for(String stage:bootUpgrade?List.of("prepare","upgrade","verify","rollback"):bootDependency?List.of("prepare","verify","removed"):packageAssets?List.of("prepare","reuse","verify"):settings?personaStages(true):bodyRecovery?bodyRecoveryStages():opacityPersistence?opacityPersistenceStages():persona||conversation||audience||events||schedule||shared||sharedAgent||clientScript||clientJava?personaStages(true):stages(recovery)){
            ...
            if(conversationAgentReal){if(System.getProperty("mineagent.conversationAgentScenario","").equals("agent_models"))builder.environment().put("MINEAGENT_REAL_AGENT_MODEL_TEST","true");builder.environment().put("MINEAGENT_REAL_PROVIDER_AUDIT_DIR",game.resolve("real-provider-audit").toString());String cap=System.getProperty("mineagent.conversationAgentCallBudget","");builder.environment().put("MINEAGENT_REAL_PROVIDER_MAX_CALLS",Set.of("building_files_zero","world_geometry_zero","agent_models_ui","provider_models","feedback_native_zero").contains(System.getProperty("mineagent.conversationAgentScenario",""))?"0":Set.of("python_host_saved","creature_events_saved","watch_ui_saved","creatures_saved","thinking_saved","runtime_item_saved","runtime_throw_saved","runtime_throw_sphere_saved","basketball_saved","management_ysm_saved").contains(System.getProperty("mineagent.conversationAgentScenario",""))?"0":cap.isBlank()?"24":cap);}
            ...
            if(!process.waitFor(conversationAgentReal?(System.getProperty("mineagent.conversationAgentScenario","").equals("world_geometry")?45:12):uiRepair?30:generatedObjects||patchResume||worldUiModel?20:bootUpgrade||bootDependency?10:6,TimeUnit.MINUTES)){process.destroyForcibly();process.waitFor(10,TimeUnit.SECONDS);throw new IllegalStateException("JOINT_APPEARANCE_TIMEOUT: "+run+" stage="+stage);}
