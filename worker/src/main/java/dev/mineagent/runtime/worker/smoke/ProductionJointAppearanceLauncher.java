...
public final class ProductionJointAppearanceLauncher {
    ...
    static void validateConversationAgentMode(Map<?,?> flags){
        ...
        if(!Set.of("","feedback","feedback_remaining","feedback_interrupt","feedback_native_zero","management","management_ysm","management_ysm_saved","autonomy","host","primitives","basketball","basketball_retry","basketball_layout","diamond","rules","cobble","blueprint","web","interaction","runtime_item","runtime_item_saved","runtime_throw","runtime_throw_saved","runtime_throw_sphere","runtime_throw_sphere_saved","basketball_saved").contains(scenario)||!real&&(!scenario.isBlank()||!budget.isBlank()))throw new IllegalArgumentException("CONVERSATION_AGENT_REAL_SCENARIO");
    ...
    public static void main(String[] args)throws Exception{
        ...
        for(String stage:bootUpgrade?List.of("prepare","upgrade","verify","rollback"):bootDependency?List.of("prepare","verify","removed"):packageAssets?List.of("prepare","reuse","verify"):settings?personaStages(true):bodyRecovery?bodyRecoveryStages():opacityPersistence?opacityPersistenceStages():persona||conversation||audience||events||schedule||shared||sharedAgent||clientScript||clientJava?personaStages(true):stages(recovery)){
            ...
            if(conversationAgentReal){builder.environment().put("MINEAGENT_REAL_PROVIDER_AUDIT_DIR",game.resolve("real-provider-audit").toString());String cap=System.getProperty("mineagent.conversationAgentCallBudget","");builder.environment().put("MINEAGENT_REAL_PROVIDER_MAX_CALLS",System.getProperty("mineagent.conversationAgentScenario","").equals("feedback_native_zero")?"0":Set.of("runtime_item_saved","runtime_throw_saved","runtime_throw_sphere_saved","basketball_saved","management_ysm_saved").contains(System.getProperty("mineagent.conversationAgentScenario",""))?"0":cap.isBlank()?"24":cap);}
