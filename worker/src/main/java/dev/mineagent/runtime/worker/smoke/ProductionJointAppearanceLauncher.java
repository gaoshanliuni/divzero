...
public final class ProductionJointAppearanceLauncher {
    ...
    public static void main(String[] args)throws Exception{
        ...
        for(String stage:bootUpgrade?List.of("prepare","upgrade","verify","rollback"):bootDependency?List.of("prepare","verify","removed"):packageAssets?List.of("prepare","reuse","verify"):settings?personaStages(true):bodyRecovery?bodyRecoveryStages():opacityPersistence?opacityPersistenceStages():persona||conversation||audience||events||schedule||shared||sharedAgent||clientScript||clientJava?personaStages(true):stages(recovery)){
            ...
            if(conversationAgentReal){builder.environment().put("MINEAGENT_REAL_PROVIDER_AUDIT_DIR",game.resolve("real-provider-audit").toString());String cap=System.getProperty("mineagent.conversationAgentCallBudget","");builder.environment().put("MINEAGENT_REAL_PROVIDER_MAX_CALLS",Set.of("provider_models","feedback_native_zero").contains(System.getProperty("mineagent.conversationAgentScenario",""))?"0":Set.of("watch_ui_saved","creatures_saved","thinking_saved","runtime_item_saved","runtime_throw_saved","runtime_throw_sphere_saved","basketball_saved","management_ysm_saved").contains(System.getProperty("mineagent.conversationAgentScenario",""))?"0":cap.isBlank()?"24":cap);}
