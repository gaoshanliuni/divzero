...
public final class ProductionJointAppearanceLauncher {
    ...
    static void validateConversationAgentMode(Map<?,?> flags){
        ...
        if(!Set.of("","rules","cobble","blueprint","web","interaction").contains(scenario)||!real&&(!scenario.isBlank()||!budget.isBlank()))throw new IllegalArgumentException("CONVERSATION_AGENT_REAL_SCENARIO");
        ...
        if(!active)return;
        var allowed=Set.of("mineagent.conversationAgentBlueprint","mineagent.conversationAgentScenario","mineagent.conversationAgentCallBudget","mineagent.conversationAgentSmoke","mineagent.conversationAgentReal","mineagent.conversationAgentProviderDb","mineagent.worldUiFixture","mineagent.worldUiAgentFixture");
    ...
    public static void main(String[] args)throws Exception{
        ...
        for(String stage:bootUpgrade?List.of("prepare","upgrade","verify","rollback"):bootDependency?List.of("prepare","verify","removed"):packageAssets?List.of("prepare","reuse","verify"):settings?personaStages(true):bodyRecovery?bodyRecoveryStages():opacityPersistence?opacityPersistenceStages():persona||conversation||audience||events||schedule||shared||sharedAgent||clientScript||clientJava?personaStages(true):stages(recovery)){
            ...
            if(conversationAgentReal){builder.environment().put("MINEAGENT_REAL_PROVIDER_AUDIT_DIR",game.resolve("real-provider-audit").toString());String cap=System.getProperty("mineagent.conversationAgentCallBudget","");builder.environment().put("MINEAGENT_REAL_PROVIDER_MAX_CALLS",cap.isBlank()?"24":cap);}
