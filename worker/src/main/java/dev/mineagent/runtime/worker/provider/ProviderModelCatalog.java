...
public final class ProviderModelCatalog {
    ...
    public static Result fetch(String base,String key){
        try {
            ...
            if(key==null||key.isBlank())return new Result(List.of(),"MODELS_KEY_REQUIRED",0);
            try(var client=HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(8)).followRedirects(HttpClient.Redirect.NEVER).build()){
                var request=HttpRequest.newBuilder(url).timeout(Duration.ofSeconds(15)).header("Accept","application/json").header("Authorization","Bearer "+key).GET().build();
                ...
                for(var row:rows){var node=row.get("id");if(node==null||!node.isTextual())return new Result(List.of(),"MODELS_INVALID_RESPONSE",status);String id=node.textValue();
                    if(id.isBlank()||id.length()>256||id.codePoints().anyMatch(Character::isISOControl)||id.contains(key)||id.startsWith("sk-"))return new Result(List.of(),"MODELS_INVALID_RESPONSE",status);models.add(id);
                }
