...
public record RuntimeItemBinding(UUID world,UUID instance,String part,String packageHash,String modelPath,String assetHash,String modelSource,int chargeTicks) {
    ...
    public RuntimeItemBinding {
        ...
        if(chargeTicks<0||chargeTicks>200)throw new IllegalArgumentException("RUNTIME_ITEM_CHARGE_BOUNDS");
        ...
        var mesh=bundle.mesh();if(mesh.collision().nonSolid())throw new IllegalArgumentException("RUNTIME_ITEM_REQUIRES_SOLID");if(mesh.vertices().size()>RuntimeMesh.MAX_VERTICES||mesh.triangles().size()>RuntimeMesh.MAX_TRIANGLES||mesh.vertices().stream().anyMatch(v->v.x()<-.5||v.x()>.5||v.y()<0||v.y()>1||v.z()<-.5||v.z()>.5))throw new IllegalArgumentException("RUNTIME_ITEM_MODEL_BOUNDS");
    }
    ...
    public static RuntimeItemBinding create(UUID world,UUID instance,String part,String hash,String path,String source,int chargeTicks){return new RuntimeItemBinding(world,instance,part,hash,path,RuntimeModelBundle.create(source.getBytes(StandardCharsets.UTF_8),new byte[0]).sha256(),source,chargeTicks);}
