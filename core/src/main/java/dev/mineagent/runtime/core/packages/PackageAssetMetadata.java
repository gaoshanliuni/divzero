...
public final class PackageAssetMetadata {
    ...
    public record Input(UUID operation,UUID world,UUID owner,String action,UUID source,long sourceRevision,String sourceHash,String name,UUID shelf,long expectedRevision){
        public Input{Objects.requireNonNull(operation);Objects.requireNonNull(world);Objects.requireNonNull(owner);Objects.requireNonNull(source);if(!Set.of("RENAME","COPY","SAVE_ASSET","REUSE_ASSET","WITHDRAW_ASSET","RESTORE_ASSET").contains(action)||sourceRevision<1||sourceHash==null||!sourceHash.matches("[a-f0-9]{64}")||name==null||name.length()>128||name.codePoints().anyMatch(c->Character.isISOControl(c))||expectedRevision<0||Set.of("COPY","REUSE_ASSET").contains(action)&&name.isBlank()||Set.of("REUSE_ASSET","WITHDRAW_ASSET","RESTORE_ASSET").contains(action)&&shelf==null||Set.of("COPY","RENAME","SAVE_ASSET").contains(action)&&shelf!=null)throw new IllegalArgumentException("PACKAGE_ASSET_INPUT");}
