...
public final class GeometrySpool implements AutoCloseable {
    ...
    public WorldGeometry.StreamSummary generate(String source,BooleanSupplier permit)throws Exception{
        db.setAutoCommit(false);try(var p=db.prepareStatement("INSERT INTO cells(x,y,z,state,orientation) VALUES(?,?,?,?,?) ON CONFLICT(x,y,z) DO UPDATE SET state=excluded.state,orientation=excluded.orientation")){
            int[] pending={0};var result=WorldGeometry.stream(source,c->{try{if(!permit.getAsBoolean())throw new IllegalStateException("GEOMETRY_CANCELLED");p.setInt(1,c.pos().x());p.setInt(2,c.pos().y());p.setInt(3,c.pos().z());p.setString(4,c.state());p.setString(5,String.join(",",c.orientation()));p.addBatch();if(++pending[0]==4096){if(Files.getFileStore(file).getUsableSpace()<256L*1024*1024)throw new IllegalStateException("GEOMETRY_DISK_SPACE_REQUIRED");p.executeBatch();db.commit();pending[0]=0;}}catch(Exception e){throw new IllegalStateException("GEOMETRY_SPOOL_WRITE_FAILED",e);}});p.executeBatch();db.commit();return result;
