...
public record CreatureDefinition(String name,String disposition,double health,double speed,double damage,
        ...
        double proximityRadius,String proximityMessage,int proximityCooldown,List<ProximityAction> proximityActions,String model,String source) {
    ...
    public static CreatureDefinition parse(String source){try{
        ...
        return new CreatureDefinition(name,disposition,num(n,"health",20,1,1024),num(n,"speed",.25,.05,1),num(n,"damage",3,0,100),food,bool(n,"rideable"),bool(n,"companion"),List.copyOf(trades),barter,radius,message,cooldown,List.copyOf(actions),model,JSON.writeValueAsString(n));
