...
public record CreatureDefinition(String name,String disposition,double health,double speed,double damage,
        ...
        double proximityRadius,String proximityMessage,int proximityCooldown,List<ProximityAction> proximityActions,String model,Map<String,CreatureAnimation.Clip> animations,String source) {
    ...
    public static CreatureDefinition parse(String source){try{
        if(source==null||source.length()>12000)throw new IllegalArgumentException("CREATURE_DEFINITION_SIZE");JsonNode n=JSON.readTree(source);keys(n,"name","disposition","health","speed","damage","food","rideable","companion","trades","barter","proximity","model","animations");
        ...
        return new CreatureDefinition(name,disposition,num(n,"health",20,1,1024),num(n,"speed",.25,.05,1),num(n,"damage",3,0,100),food,bool(n,"rideable"),bool(n,"companion"),List.copyOf(trades),barter,radius,message,cooldown,List.copyOf(actions),model,CreatureAnimation.parse(n.path("animations")),JSON.writeValueAsString(n));
