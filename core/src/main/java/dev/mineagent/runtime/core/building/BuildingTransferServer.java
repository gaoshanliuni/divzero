...
public final class BuildingTransferServer implements AutoCloseable {
    private static final ObjectMapper JSON=new ObjectMapper();private final HttpServer server;private final ExecutorService executor;private final BuildingFileStore store;
    ...
    private void serve(HttpExchange x)throws IOException{try(x){try{
        ...
        if(method.equals("GET")&&path.startsWith("/download/")){UUID id=UUID.fromString(path.substring(10));var a=store.asset(owner,id);h.set("Content-Type","application/octet-stream");h.set("Content-Disposition","attachment; filename*=UTF-8''"+URLEncoder.encode(a.name()+(a.kind().equals("WORLD")?".zip":""),StandardCharsets.UTF_8).replace("+","%20"));x.sendResponseHeaders(200,0);store.archive(owner,id,x.getResponseBody());return;}
