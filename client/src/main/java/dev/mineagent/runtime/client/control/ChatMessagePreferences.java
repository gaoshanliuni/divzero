...
public final class ChatMessagePreferences {
    ...
    private ChatMessageDisplay.State load()throws Exception{
        ...
        if(!java.util.Set.of("limit","mark","revision").containsAll(p.stringPropertyNames()))throw new IllegalArgumentException("CHAT_MESSAGES_SETTINGS_INVALID");
        return new ChatMessageDisplay.State(Integer.parseInt(p.getProperty("limit","1024")),p.getProperty("mark","time"),Long.parseLong(p.getProperty("revision","0")));
    }
    public ChatMessageDisplay.State state(){return state;}
    public synchronized ChatMessageDisplay.State save(Long expected,Integer limit,String mark)throws Exception{
        ...
        var next=new ChatMessageDisplay.State(limit==null?current.limit():limit,mark==null?current.mark():mark,Math.addExact(current.revision(),1));
