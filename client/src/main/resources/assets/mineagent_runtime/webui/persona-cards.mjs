...
export function createPersonaCards({windowFor,send,report,persist}){
  ...
  async function load(discard=false){
    const agent=document.querySelector('#persona-agent')?.value,fields=document.querySelector('#persona-fields');if(!fields)return;if(!agent){++epoch;fields.replaceChildren();add('p','请选择有编辑权限的 AI。',fields);return;}
    ...
    const status=add('p','读取人设…',fields);status.id='persona-status';status.setAttribute('role','status');
    try{const r=await send('personaAction',{action:'read',agentId:agent});if(turn!==epoch||!fields.isConnected)return;if(r.code!=='OBSERVED')throw new Error(r.values?.errorCode||r.code);
      const saved=JSON.parse(r.values.state);draft.observe(saved);status.textContent=`已保存 r${saved.revision}${draft.dirty?' · 本地草稿未保存':''}`;
      const note=add('p','同一 AI 的所有会话使用其当前人设。只影响后续回复，不授予权限、不修改历史或世界任务。',fields);note.className='muted';
      const label=add('label','自由人设 · 身份 / 背景 / 性格 / 说话风格 / 与玩家的关系 / 角色扮演要求',fields);label.htmlFor='persona-text';
      const input=add('textarea',null,fields);input.id='persona-text';input.maxLength=8192;input.rows=5;input.value=draft.text;input.placeholder='留空使用默认 AI 玩家身份；最多 8192 个字符。';
      input.oninput=()=>{draft.edit(input.value);persist();};const actions=add('div',null,fields);actions.className='actions';
      const save=add('button','保存人设',actions);save.id='persona-save';const reset=add('button','恢复默认',actions);reset.id='persona-reset';const refresh=add('button','重新读取（放弃该 AI 草稿）',actions);refresh.id='persona-refresh';
      ...
      async function commit(defaults){save.disabled=reset.disabled=refresh.disabled=true;try{
        ...
        input.value=draft.text;status.textContent=`已保存 r${draft.revision}${draft.dirty?' · 仍有新草稿未保存':defaults?' · 默认 AI 玩家身份':''}`;fields.dataset.personaRevision=String(draft.revision);fields.dataset.personaAgent=agent;persist();
      }catch(e){if(turn===epoch){status.textContent=`${e.message}；草稿保留，请核对后重新读取。`;report(e);}}finally{if(turn===epoch)save.disabled=reset.disabled=refresh.disabled=false;}}
      save.onclick=()=>commit(false);reset.onclick=()=>commit(true);
    }catch(e){if(turn===epoch){status.textContent=e.message;report(e);}}
  }
  function open(){const c=windowFor('persona','AI 人设');if(c.childElementCount)return;add('p','每个 AI 独立保存，所有者及授权协作者可编辑。',c);const s=add('select',null,c);s.id='persona-agent';s.setAttribute('aria-label','人设所属 AI');s.onchange=()=>load();add('div',null,c).id='persona-fields';fill();load();}
