...
export function createJavaStudio({windowFor,send,openCatalog,openNativeApi,openClientScripts,nativeApiContext}) {
  ...
  async function list(){
    ...
    try{
      ...
      el('p','读取本人真实旧/新草稿。保存、发布源码、SERVER执行或CLIENT本机下载/执行分别操作；不自动调用模型或重放未知执行。',root,'muted');
      const actions=el('div',null,root,'actions');
      for(const type of ['js','mjs','java'])button('新建 SERVER '+type,actions,()=>newDraft(type,'SERVER'));
      for(const type of ['js','mjs','java'])button('新建 CLIENT '+type,actions,()=>newDraft(type,'CLIENT'));
      button('Coder 请求与候选历史',actions,()=>{newDraft('js');coderTools?.openHistory();});
      button('包目录',actions,openCatalog);button('Native API / 编译来源',actions,openNativeApi);
      if(!value.drafts.length)el('p','暂无可读取草稿；请选择真实 Agent 并填写源码创建，不运行占位示例。',root,'muted');
      for(const d of value.drafts){
        ...
        el('p','草稿 '+d.id+' · Package '+d.packageId,card,'muted');button('读取 / 编辑 / 发布',card,()=>void loadDraft(d.id));
      }
      const pages=el('div',null,root,'actions');
      button('上一页',pages,()=>{offset=Math.max(0,offset-8);void list();},offset>0);
      button('下一页',pages,()=>{offset=value.nextOffset;void list();},value.more);
    }catch(error){if(current(e,ticket)){releaseEditor();root.replaceChildren();el('p',error.message,root,'error');button('重读列表',root,()=>void list());}}
  ...
  function renderEditor(){
    releaseEditor();root.replaceChildren();renderedRuns='';el('h3',draft?'Code Studio · 编辑与生命周期':'Code Studio · 新草稿',root);
    el('p',isClient()?'CLIENT源码发布后不会在服务器执行；Rhino使用client host，Java主类实现ClientRuntimeExtension。发布、下载、本机确认与运行分开；本机原生代码可访问文件/网络/剪贴板/账号可见数据。':'SERVER Java使用RuntimeExtension；Rhino使用host/on/schedule/track。可调用真实Java，不是强制沙箱。',root,'warning');
    const actions=el('div',null,root,'actions');
    button('草稿列表（放弃未保存编辑）',actions,()=>{dirty=false;void list();});
    button('包目录',actions,openCatalog);button('查看 Native API',actions,openNativeApi);
    const labels=el('div',null,root,'event-toolbar');
    const pathLabel=el('label','源码路径',labels);pathInput=el('input',null,pathLabel);pathInput.value=path;pathInput.maxLength=128;pathInput.disabled=!!draft;
    const nameLabel=el('label','统一包名称',labels);nameInput=el('input',null,nameLabel);nameInput.value=packageName;nameInput.maxLength=128;nameInput.placeholder='发布源码时使用';
    const agentLabel=el('label','新任务归属 Agent',labels);agentSelect=el('select',null,agentLabel);
    el('option','请选择可管理的真实 Agent',agentSelect).value='';for(const a of agents)el('option',a.name,agentSelect).value=a.id;
    editor=el('textarea',null,root,'event-json');editor.value=source;editor.maxLength=16000;editor.rows=18;editor.spellcheck=false;editor.setAttribute('aria-label','Java 或 Rhino 源码');
    baseText=editor.value;
    editor.oninput=()=>{source=editor.value;dirty=editor.value!==baseText;report('源码已编辑；先前检查不适用于此版本。');renderStatus();};
    ...
    if(isClient())el('p','CLIENT Coder 上下文尚未接入本机依赖与 CLIENT 编译来源；本批仅允许手工编辑，避免把 SERVER Coder 结果误标为 CLIENT。',root,'muted');
    ...
    workspaceTools=attachStudioWorkspace({container:root,send,available:()=>visible()&&!busy,notify:report,
      ...
      onChanged:async result=>{if(draft?.id!==result.draftId)return;if(dirty){report('工作区已保存，期间新的主文件编辑保留；请明确重载后再发布/运行。');await observe();return;}const desiredName=nameInput.value,changedName=desiredName!==packageName;await loadDraft(result.draftId);if(changedName&&nameInput?.isConnected)nameInput.value=desiredName;}});
    pathInput.addEventListener('input',()=>editorTools?.schedulePreview());
    const writing=el('div',null,root,'actions');
    button('检查当前 Rhino 源码（不执行）',writing,()=>void checkSource());
    if(!draft)button('创建持久草稿与任务',writing,()=>void act('create'));
    else{
      button('保存草稿',writing,()=>void act('save'),latest.status==='DRAFT').dataset.writeAction='save';
      button('发布为统一 RuntimePackage 源码',writing,()=>void act('publishSource')).dataset.writeAction='publishSource';
      button('读取统一包另建新稿（放弃本地编辑）',writing,()=>void openPackage(packageId,targetSide,isJava()?'JAVA':'RHINO'),packageRevision>0).dataset.studioAction='newFromPackage';
      if(isClient()){const download=button('下载已发布 CLIENT 源码（不执行）',root,()=>void downloadClient(),packageRevision>0&&!!packageCanonical);download.dataset.clientDownload='true';}
      else{const label=el('label',null,root),consent=el('input',null,label);consent.type='checkbox';
        label.append(document.createTextNode(isJava()?'明确编译并调用 Java start；可能产生真实世界或外部副作用。':'明确执行 Rhino 顶层并允许后续 tick/schedule；关闭界面或退出不会代替停止。'));
        button(isJava()?'编译并启动此保存版本':'执行此保存的 Rhino 版本',root,()=>{if(!consent.checked){report('请先确认 Native 执行。');return;}void act('run');},latest.status==='DRAFT').dataset.writeAction='run';}
      button('重载服务器草稿（放弃本地编辑）',root,()=>void loadDraft(draft.id));
      button('查看保留旧稿（只读）',root,()=>void history(0));
    }
    el('p',isClient()?'发布源码不等于下载或运行；下载完成仍须在本机管理器再次确认。依赖也不会自动下载、批准或启动。':'发布源码不等于运行。更新源包会使旧执行在后续授权检查中失效；停止只清理受管句柄，不承诺回滚副作用。',root,'muted');
  ...
  async function history(offset){
    ...
    area.replaceChildren();el('p','读取原 CodeDraft 保存的历史文本，不执行旧稿…',area,'muted');
    try{
      ...
      area.replaceChildren();el('h3','保留旧稿 · '+info.total+' 项',area);
      el('p','原记录只保留最近最多 20 次保存前文本；序号不是历史 revision 或时间，不补造已丢失历史。采用只改本地编辑，仍需明确保存。',area,'muted');
      if(!info.history.length)el('p','没有保留旧稿。',area,'muted');
      for(const item of info.history){const card=el('article',null,area,'event-card');el('strong','保留序号 '+(item.index+1)+' · '+item.characters+' 字符',card);el('p',item.hash,card,'muted');button('读取全文 / 与当前编辑对照',card,()=>void historySource(info,item,card));}
      const pages=el('div',null,area,'actions');button('更早的保留稿',pages,()=>void history(Math.max(0,offset-8)),offset>0);button('后面的保留稿',pages,()=>void history(info.nextOffset),info.more);
    }catch(error){if(current(e,ticket)&&read===historySerial&&area.isConnected){area.replaceChildren();el('p',error.message+'；草稿可能已更新，请重新读取。',area,'error');}}
  }
  async function historySource(info,item,card){
    ...
    try{
      ...
      card.replaceChildren();el('strong','保留序号 '+(item.index+1)+' · SHA '+item.hash,card);
      el('p','两侧原文对照，不自动合并；当前编辑快照仅用于本次比较。'+(text===stamp?'两份文本相同。':''),card,'muted');
      const grid=el('div',null,card,'studio-source-comparison');const old=el('section',null,grid),local=el('section',null,grid);
      el('h4','保留旧稿',old);el('pre',text,old,'event-json');el('h4','读取时的本地编辑',local);el('pre',stamp,local,'event-json');
      if(latest?.status==='DRAFT'){
        const label=el('label',null,card),confirm=el('input',null,label);confirm.type='checkbox';label.append(document.createTextNode('确认用此旧稿替换当前本地编辑（可在本窗口撤销，不自动保存或运行）。'));
        button('载入本地编辑',card,()=>{
          if(!confirm.checked)return;
          if(!currentRead()||draft?.id!==info.draftId||baseRevision!==info.revision||latest?.revision!==info.revision||latest.status!=='DRAFT'||editor.value!==stamp){report('草稿或本地编辑已变化，请重新读取旧稿并确认。');return;}
          if(editorTools?.replaceDocument(text))report('旧稿已载入本地；尚未保存、发布或执行。');
        });
      }else el('p','已发布草稿只读。需要修改时先从统一包另建编辑稿。',card,'muted');
    }catch(error){if(currentRead())el('p',error.message,card,'error');}
  }
  function renderStatus(){
    if(!status?.isConnected)return;lock();
    if(!latest){
      status.textContent=packageId?'基于 Package '+packageId+' @ r'+packageRevision+'，尚未创建新草稿。':'尚未创建草稿；不会自动生成或执行样例。';return;
    }
    status.textContent='草稿 '+latest.id+' · '+(latest.targetSide||targetSide)+' · '+latest.status+' r'+latest.revision+' · 包库 r'+latest.packageRevision
      +' · '+(latest.fileCount||1)+' 个源码文件 · '+(latest.dependencyCount||0)+' 直接依赖'
      +(latest.nativePublicationContext?' · 发布将复用冻结 Native raw '+(latest.nativeTypeCount||0)+' / overlay '+(latest.nativeOverlayCount||0):latest.nativeContextInvalidated?' · 源码已改，原 Native context 失效；发布将重新按 raw 编译':'')
      +(dirty?' · 未保存的本地编辑保留':latest.revision!==baseRevision?' · 服务器版本已变化，请明确重载':'')
      +'；library enabled 不等于当前执行状态。';
    ...
    if(latest.legacyActive){
      const card=el('article',null,runs,'event-card');el('strong','旧 RuntimePackageManager · 当前句柄 '+latest.legacyLoaded,card);
      el('p','不是新的 Studio publication。必须先停止准确旧脚本，才能运行同包的新源码；不自动迁移运行状态。',card,'warning');
      if(latest.legacyCanStop){
        const label=el('label',null,card),confirm=el('input',null,label);confirm.type='checkbox';
        label.append(document.createTextNode('确认停止与此旧稿 Task / 源码 / 包版本完全匹配的脚本，并停用对应旧发布记录。'));
        button('停止匹配的旧脚本',card,()=>{if(confirm.checked)void stopLegacy();});
      }else el('p','此草稿与当前旧运行来源不匹配；请读取原草稿，不以新源码代替停止目标。',card,'muted');
    }
    for(const r of latest.publications){
      const card=el('article',null,runs,'event-card'),rhino=r.language==='RHINO';
      el('strong',(rhino?'Rhino':'Java')+' · '+r.state+' · 当前句柄 '+r.loaded+(r.suspended?' · 已停止 callback 分派':''),card);
      el('p',r.id+' · '+r.error+(r.uncertain?' · 存在未知影响':'')+(r.metadataPending?' · 回执待收束，不重新执行':''),card,'muted');
      el('p','源 Package SHA '+(r.packageHash||'旧记录无统一包 SHA')+(rhino?' · source '+r.sourceHash:' · artifact '+r.artifact+' · classpath '+r.nativeClasspath),card,'muted');
      el('p','冻结依赖 '+(r.dependencyCount||0)+' · graph '+(r.dependencyHash||'无')+' · 被 '+(r.consumerCount||0)+' 个 publication 引用（须先停止消费者）',card,'muted');
      if(r.legacyStop)el('p','旧脚本停止回执，不是新脚本执行。',card,'muted');
      if(rhino&&r.source)el('p',r.source+':'+r.line+':'+r.column,card,'error');
      const details=el('section',null,card);button(rhino?'读取脚本故障位置':'读取保留的编译诊断',card,()=>void diagnostics(r,0,details));
      ...
      if(!r.legacyStop&&stoppable.includes(r.state)){
        const label=el('label',null,card),confirm=el('input',null,label);confirm.type='checkbox';
        label.append(document.createTextNode('仅停止此 publication，不宣称世界或外部副作用已回滚。'));
        button('停止原执行',card,()=>{if(confirm.checked)void stop(r);});
      }
    }
    if(!latest.publications.length)el('p','此页没有执行记录；保存或源码发布不会自动创建执行。',runs,'muted');
    const pages=el('div',null,runs,'actions');
    button('较新执行记录',pages,()=>void publicationPage(Math.max(0,runsOffset-8)),runsOffset>0);
    button('较早执行记录',pages,()=>void publicationPage(latest.nextPublicationOffset),latest.morePublications);
  }
  async function diagnostics(run,offset,area){
    ...
    try{
      ...
      el('pre',value.text||'没有保留的诊断；状态与未知影响以原执行记录为准。',area,'event-json');
      button('上一段',area,()=>void diagnostics(run,Math.max(0,offset-4096),area),offset>0);
      button('下一段',area,()=>void diagnostics(run,value.nextOffset,area),value.more);
    }catch(error){if(current(e,ticket)&&area.isConnected)el('p',error.message,area,'error');}
  }
  async function checkSource(){
    if(busy||!visible())return;
    if(!/\.(m?js)$/i.test(pathInput.value)){report('此检查只适用于 Rhino JS/mjs；Java 使用明确的编译入口。');return;}
    ...
    try{
      ...
      report(result.accepted?'Rhino preflight 接受；未执行，不证明 Native API 或实际效果正确。'
  ...
  async function downloadClient(){
    ...
    try{const result=await send('clientScript',{kind:isJava()?'downloadJava':'download',packageId,packageRevision:String(packageRevision),canonical:packageCanonical});if(!current(e,ticket))return;if(result.code!=='ACCEPTED'&&!String(result.code).endsWith('DOWNLOADED_NOT_APPROVED'))throw new Error(result.code||'CLIENT_CODE_DOWNLOAD_FAILED');report('已受理本机下载；不会编译或执行。请在本机代码管理器读取状态并另行确认启动。');openClientScripts?.(packageId);}
    catch(error){if(current(e,ticket))report(error.message+'；不自动重发。');}
    finally{if(e===epoch){busy=false;lock();}}
  }
  async function act(action){
    if(busy||!visible())return;
    if(workspaceTools?.hasUnsaved()){report('工作区文件有未保存编辑，请先保存或明确放弃，避免只使用旧文件集。');return;}
    if((action==='run'||action==='publishSource')&&dirty){report('请先保存源码，再发布或运行。');return;}
    ...
    }catch(error){if(current(e,ticket))report(error.message+'；请只读核查原记录，不自动重发。');}
    finally{if(e===epoch){busy=false;lock();}}
  }
  async function stop(run){
    ...
    }catch(error){if(current(e,ticket))report(error.message+'；停止结果以原账本为准，不自动重复 cleanup。');}
    finally{if(e===epoch){busy=false;lock();}}
  }
  async function stopLegacy(){
    ...
    }catch(error){if(current(e,ticket))report(error.message+'；旧执行不自动重放或替换。');}
    finally{if(e===epoch){busy=false;lock();}}
  }
  ...
  function session(value){
    ...
    releaseEditor();root?.replaceChildren();if(root?.isConnected)el('p','上下文变化，请重新打开 Code Studio；旧回复不创建新稿或运行。',root,'muted');
  }
