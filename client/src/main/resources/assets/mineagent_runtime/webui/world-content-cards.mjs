...
const activationStatus=a=>__uiF("{0} · {1} · 重启恢复 {2}",a.state,a.instanceId,a.state==='DISABLED'?__uiT("已停止，不恢复"):a.autoRestore?__uiT("已允许"):__uiT("未允许"));
...
export function createWorldContentCards({windowFor,send,report,openRestore}) {
  ...
  async function manage(page=0){
    const content=windowFor('runtime-world-instances',__uiT("已有世界实例"));const token=++pageToken;content.replaceChildren();
    const status=add('p',__uiT("读取属于你的实际世界实例…"),content);status.setAttribute('role','status');
    try{const r=await send('packageAction',{action:'worldList',page});if(r.code!=='OBSERVED')throw new Error(r.values?.errorCode||r.code);if(!content.isConnected||token!==pageToken)return;
      const result=JSON.parse(r.values.page);status.textContent=__uiF("{0} 个实例 · {1}/{2} 页",result.count,result.page+1,result.pages);
      const previous=add('button',__uiT("上一页实例"),content);previous.disabled=result.page<=0;previous.onclick=()=>manage(result.page-1);
      const next=add('button',__uiT("下一页实例"),content);next.disabled=result.page+1>=result.pages;next.onclick=()=>manage(result.page+1);
      const refresh=add('button',__uiT("只读刷新列表"),content);refresh.onclick=()=>manage(result.page);
      for(const a of result.instances){const row=add('article',null,content);row.className='generation-job';row.dataset.managedInstance=a.instanceId;add('strong',a.packageName,row);add('p',`${a.state} · ${a.instanceId}`,row);add('p',`${a.location.dimension} · ${a.location.x}, ${a.location.y}, ${a.location.z}`,row).className='muted';
        if(a.error)add('p',__uiT("原因：")+a.error,row).className='muted';if(openRestore){const restore=add('button',__uiT("原实例恢复 / 诊断"),row);restore.onclick=()=>openRestore(a.activationId);}
        const move=add('button',__uiT("移动这个实例"),row);move.dataset.worldMoveOpen=a.instanceId;move.disabled=!a.movable;move.onclick=()=>moveForm(a.activationId,a.instanceId);
  ...
  async function moveForm(activationId,instanceId){
    const content=windowFor(`move-${instanceId}`,__uiT("只移动所选实例"));if(content.childElementCount)return;
    const status=add('p',__uiT("读取准确实例与位置…"),content);status.setAttribute('role','status');
    ...
    try{const r=await send('packageAction',{action:'worldMoveInspect',activationId});if(r.code!=='OBSERVED')throw new Error(r.values?.errorCode||r.code);if(!content.isConnected)return;const a=JSON.parse(r.values.instance);
      ...
      add('p',__uiF("原点 {0}: {1}, {2}, {3}；仅平移这个实例，不修改定义或同类实例。",a.location.dimension,a.location.x,a.location.y,a.location.z),content).className='muted';
      add('p',__uiT("保持实体 UUID、脚本监听和计时器。当前支持同维度 64 格内的受管 Mesh 与普通完整碰撞方块；含方块实体、流体或特殊形状须适配，不能丢弃数据强行移动。未知结果不会自动重放。"),content).className='muted';
      const grid=add('div',null,content);grid.className='world-move-coordinates';const fields={};for(const key of ['x','y','z']){const cell=add('div',null,grid),label=add('label',__uiF("目标原点 {0}",key.toUpperCase()),cell);const input=add('input',null,cell);input.id=`world-move-${instanceId}-${key}`;label.htmlFor=input.id;input.type='number';input.step='any';input.value=a.location[key];input.dataset.worldMoveField=key;fields[key]=input;}
      const label=add('label',null,content),consent=add('input',null,label);consent.type='checkbox';consent.dataset.worldMoveConsent=instanceId;label.append(document.createTextNode(__uiT("明确移动此实例的受管部件与锚点；不声称可迁移任意脚本的外部 Java 副作用")));
      const submission=new WorldMoveSubmission();let completed=false;const move=add('button',__uiT("确认只移动此实例"),content);move.dataset.worldMoveApply=instanceId;
      move.onclick=async()=>{move.disabled=true;status.setAttribute('role','status');status.className='';try{const values=submission.request(a,Object.fromEntries(Object.entries(fields).map(([k,v])=>[k,v.value])),consent.checked,()=>crypto.randomUUID());status.dataset.worldMoveRequest=JSON.stringify(values);const receipt=await send('packageAction',{action:'worldMove',...values});if(receipt.code!=='APPLIED')throw new Error(receipt.values?.errorCode||receipt.code);const result=JSON.parse(receipt.values.move);status.textContent=__uiF("{0} · 当前原点 {1}, {2}, {3} · 实例 r{4}{5}",result.receipt.state,result.location.x,result.location.y,result.location.z,result.instanceRevision,result.duplicate?__uiT(" · 持久去重回执"):'');status.className='success';completed=true;move.textContent=__uiT("已移动 · 实体身份保留");}catch(e){status.textContent=e.message;status.className='error';status.setAttribute('role','alert');report(e);}finally{if(!completed)move.disabled=false;}};
      const reload=add('button',__uiT("重新读取（放弃未提交位置）"),content);reload.dataset.worldMoveRefresh=instanceId;reload.onclick=()=>{content.replaceChildren();moveForm(activationId,instanceId);};
    }catch(e){status.textContent=e.message;report(e);}
  }
  async function open(job,revision){
    const content=windowFor(`world-${job.packageId}-r${revision}`,__uiT("世界内容 · 明确启用"));
    if(content.childElementCount)return;
    const status=add('p',__uiT("读取签名包与实例状态…"),content);status.setAttribute('role','status');
    try{
      ...
      status.textContent=__uiF("签名包 r{0} · {1}；生成完成不代表已经放置。",revision,r.values.activationMode);
      add('p',__uiT("明确启用后执行包内 Rhino/Java 原生代码，需要 RUN_CODE 和 MANAGE_PACKAGES。关闭此界面不停止已启用内容；停用只清理受管脚本，保留世界块与实例数据。"),content).className='muted';
      const fields={};
      for(const [key,label] of [['definitionId',__uiT("实例定义")],['dimension',__uiT("目标维度")],['x',__uiT("原点 X")],['y',__uiT("原点 Y")],['z',__uiT("原点 Z")]]){
      ...
      const label=add('label',null,content),consent=add('input',null,label);consent.type='checkbox';consent.dataset.worldConsent='true';label.append(document.createTextNode(__uiT("明确允许本次在所填位置执行原生代码并创建实例")));
      ...
      restoreLabel.append(document.createTextNode(__uiT("另行允许服务器重启后恢复此实例的相同签名代码；不会重放创建回调，可随时撤销。")));
      ...
      const start=add('button',__uiT("启用并创建世界实例"),content);start.dataset.worldActivate=job.packageId;
      start.onclick=async()=>{start.disabled=true;try{
        ...
        const a=JSON.parse(receipt.values.activation);status.textContent=__uiF("{0} · instance {1} · 当前受管原版块 {2}",a.state,a.instanceId,receipt.values.verifiedBlocks);
      ...
      for(const a of JSON.parse(r.values.activations)){
        if(openRestore){const restore=add('button',__uiT("原实例恢复 / 诊断 · ")+a.instanceId.slice(0,8),content);restore.onclick=()=>openRestore(a.operationId);}
    ...
    }catch(e){status.textContent=e.message;report(e);const retry=add('button',__uiT("重试只读加载"),content);retry.onclick=()=>{if(content.isConnected){content.replaceChildren();open(job,revision);}};}
  }
  function stopButton(a,content){const stop=add('button',__uiT("停用此实例脚本（保留世界数据）"),content);stop.dataset.worldDisable=a.operationId;stop.onclick=async()=>{stop.disabled=true;try{const r=await send('packageAction',{action:'worldDisable',activationId:a.operationId});if(r.code!=='APPLIED')throw new Error(r.values?.errorCode||r.code);updateWorldActivationStatus(content,JSON.parse(r.values.activation));stop.textContent=__uiT("已停用 · 世界块与数据保留");}catch(e){report(e);stop.disabled=false;}};}
  function restoreOff(a,content){const button=add('button',__uiT("撤销重启恢复（不停止当前运行）"),content);button.dataset.worldRestoreOff=a.operationId;button.onclick=async()=>{button.disabled=true;try{const r=await send('packageAction',{action:'worldRestoreOff',activationId:a.operationId,activationRevision:a.revision});if(r.code!=='APPLIED')throw new Error(r.values?.errorCode||r.code);updateWorldActivationStatus(content,JSON.parse(r.values.activation));button.textContent=__uiT("已撤销重启恢复");}catch(e){report(e);button.disabled=false;}};}
  return {manage,render(card,job,revision){const button=add('button',__uiT("查看并启用世界内容"),card);button.dataset.worldPackage=job.packageId;button.onclick=()=>open(job,revision);}};
}
