...
export function createNativeApiCards({windowFor,send}) {
  ...
  async function state(){if(!ready||polling||!root?.isConnected)return;const generation=epoch;polling=true;
    try{
      ...
      if(value.refresh?.operation)status.textContent+=' · 本人刷新 '+value.refresh.phase+' '+(value.refresh.error||'');
      ...
      if(pendingOperation&&own&&['READY','FAILED'].includes(own.phase)){pendingOperation='';refreshing=e.status==='CAPTURING';if(own.phase==='FAILED')return;if(own.snapshot!==e.snapshot){status.textContent+=' · 环境快照已变化，请明确重读当前状态。';return;}}
  ...
  async function refresh(){if(!ready||refreshing||!root?.isConnected)return;const context=scope,operation=crypto.randomUUID();pendingOperation=operation;refreshing=true;status.textContent='请求捕获 FML game layer 的原始 class 资源…';
    try{await request({kind:'refresh',operationId:operation});if(context!==scope||pendingOperation!==operation)return;await state();}
    catch(error){if(context!==scope||pendingOperation!==operation)return;status.textContent=error.message+' · 未自动重发捕获；可只读刷新状态。';pendingOperation='';refreshing=false;}
  }
  function renderSelected(parent){
    const section=el('section',null,parent,'event-card');el('strong','Coder Native 选择 · '+selection.size()+' / 16',section);
    el('p','raw class 仅发送声明元数据；live/transformed overlay 发送服务器冻结的准确 class bytes、provenance，以及明确选择的方法实现。全部选择绑定当前 snapshot、viewer、world 与 process epoch。',section,'muted');
    for(const item of selection.items){const row=el('div',null,section,'actions');el('span','raw · '+item.module+' · '+item.class,row);button('移除',row,()=>{selection.remove(item.module,item.class);selectionChanged();if(body?.isConnected)void query();});}
    for(const id of selection.overlays){const row=el('div',null,section,'actions');el('span','overlay · '+(overlayLabels.get(id)||id),row);button('移除',row,()=>{selection.removeOverlay(id);overlayLabels.delete(id);selectionChanged();if(body?.isConnected)void query();});}
    button('清空选择',section,()=>{selection.clear();overlayLabels.clear();selectionChanged();if(body?.isConnected)void query();},selection.size()>0);
  }
  ...
  function overlayButton(result,parent){
    if(result.selectionId){
      ...
      button(chosen?'移出 Coder overlay':'选入 Coder overlay',parent,()=>{try{if(hash(result.snapshot)&&selection.snapshot!==result.snapshot){snapshot=result.snapshot;resetSelection(snapshot);}if(chosen){selection.removeOverlay(result.selectionId);overlayLabels.delete(result.selectionId);}else{selection.addOverlay(result.selectionId);overlayLabels.set(result.selectionId,label);}selectionChanged();void query();}catch(error){status.textContent=error.message;}},hash(result.snapshot));
    }else el('p','当前结果只读，未签发 Coder overlay'+(result.selectionUnavailable?'：'+result.selectionUnavailable:hash(result.snapshot)?'。':'（raw snapshot 尚未 READY）。'),parent,'muted');
  }
  function textPages(result,parent){const pages=el('div',null,parent,'actions');button('上一段长文本',pages,()=>{textOffset=Math.max(0,textOffset-4096);void query();},textOffset>0);button('下一段长文本',pages,()=>{textOffset=result.nextTextOffset;void query();},result.textMore);}
  function methodButtons(result,parent,target){const methods=el('section',null,parent,'event-card');el('strong','本页方法实现 · 按需读取',methods);el('p','使用准确名称与 JVM descriptor；只读取已展示 selector，不初始化目标类。',methods,'muted');for(const method of result.methods||[])button(method.name+' '+method.descriptor,methods,()=>{methodName=method.name;methodDescriptor=method.descriptor;memberOffset=offset;offset=textOffset=0;parentMembersPage=target;page=target==='live_members'?'live_method_body':target==='transformed_members'?'transformed_method_body':'method_body';void query();});}
  function navRaw(parent){button('返回模块',parent,()=>{page='modules';module=className=methodName=methodDescriptor=search='';offset=textOffset=0;void query();});}
  function args(){
