...
const conversations=createConversationCards({send,report,persist:saveUiState});
...
addEventListener('mineagent:host', event => {
  try {
    ...
    else if(channel==='settingsVersion'||channel==='settingsChanged')serverSettings.changed(data);
