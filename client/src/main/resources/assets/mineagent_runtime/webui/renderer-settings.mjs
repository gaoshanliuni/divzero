...
export function createRendererSettings({windowFor,send,report}){
  function open(){
    const content=windowFor('renderer-settings',__uiT("界面渲染设置"));if(content.childElementCount)return;
    ...
    el('h2',__uiT("选择界面渲染方式"));
    el('p',__uiT("只保存此客户端偏好，不立即关闭或重建当前页面。关闭并重新打开整个 WebGUI 宿主，或重新进入游戏后生效。")).className='muted';
    const running=el('p',__uiT("读取当前宿主…"));running.id='renderer-current';
    const stored=el('p',__uiT("读取已保存偏好…"));stored.id='renderer-configured';
    const choice=el('fieldset');choice.id='renderer-choice';choice.disabled=true;const legend=document.createElement('legend');legend.textContent=__uiT("渲染方式");choice.append(legend);const radios=[];
    for(const [value,text]of [['LEGACY',__uiT("兼容合成 · 默认")],['LIVE_ATLAS',__uiT("独立 Native 合成 · 每窗口透明度（实验）")]]){const label=document.createElement('label');label.style.display='block';const radio=document.createElement('input');radio.type='radio';radio.name='renderer';radio.value=value;label.append(radio,document.createTextNode(text));choice.append(label);radios.push(radio);}
    el('p',__uiT("独立合成的 OS/IME、popup 和复杂输入仍在验证。切回兼容模式时，要求独立透明度的页面会明确拒绝，不暗中改成全局透明度。")).className='muted';
    const refresh=el('button',__uiT("重新读取配置"));refresh.id='renderer-refresh';
    const save=el('button',__uiT("保存，下次打开生效"));save.id='renderer-save';save.disabled=true;
    const status=el('p',__uiT("尚未保存"));status.id='renderer-status';status.setAttribute('role','status');
    for(const radio of radios)radio.onchange=()=>{if(radio.checked){model.choose(radio.value);status.textContent=__uiT("选择尚未保存");}};
    async function request(action){const id=model.begin();choice.disabled=refresh.disabled=save.disabled=true;
      try{const value=await send('rendererSettings',action);if(!content.isConnected||!model.accept(id,value))return;
        ...
        running.textContent=__uiF("当前宿主：{0}",s.running);stored.textContent=__uiF("已保存：{0}",s.configured);
        status.textContent=s.configured!==model.selected?__uiT("较早的选择已保存，当前选择尚未保存"):s.requiresReopen?__uiT("已保存；下次创建宿主生效。当前页面未重建。"):value.status==='RENDERER_SAVED'?__uiT("已保存；与当前宿主一致。"):__uiT("配置已读取。");
      }catch(error){if(content.isConnected&&id===model.serial){status.textContent=error.message==='WEBGUI_RENDERER_CONFLICT'?__uiT("配置已被修改。请重新读取后保存。"):error.message==='WEBGUI_RENDERER_CANCELLED'?__uiT("当前宿主已变化，请重新读取配置。"):error.message;report(error);}}
