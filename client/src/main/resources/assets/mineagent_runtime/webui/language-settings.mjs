...
export function createLanguageSettings({windowFor,send,report,persist}){
 async function open(){const root=windowFor('runtime-language',t('语言设置'));root.replaceChildren();const add=(tag,text)=>{const n=document.createElement(tag);if(text)n.textContent=text;root.append(n);return n;};
  add('h2',t('界面语言'));const choices=add('select');choices.id='ui-language-choice';choices.setAttribute('aria-label',t('界面语言'));for(const [id,label]of [['zh_cn','简体中文'],['en_us','English']]){const option=document.createElement('option');option.value=id;option.textContent=label;choices.append(option);}choices.disabled=true;
  add('p',t('默认中文，仅影响本机界面；玩家输入、AI 回复和内容包不翻译。')).className='muted';add('p',t('应用将重新打开 F2。已保存的会话和布局保留，请先保存其它表单。')).className='muted';
