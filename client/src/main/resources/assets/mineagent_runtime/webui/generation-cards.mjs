...
export function createGenerationCards({ windowFor, send, report, persist, openRestore, openDataPack, openResourcePack, openClientScript }) {
  ...
  function agentContainer(job,revision){
    const content=windowFor('runtime-container-agent','Agent 自身容器委派');content.replaceChildren();
    node('p','由选定 Agent 的真实身体、背包、原生距离与规则执行；你的客户端仅承载页面，不借用你的库存或 OP。',content).className='muted';
    ...
    const fields={};
    for(const [key,label] of [['agentId','执行 Agent'],['goal','任务'],['group','结果范围'],['itemId','物品 ID'],['name','精确物品显示名（可留空）'],['count','期望总数']]){
      ...
      if(key==='group')for(const [value,title] of [['player','Agent 背包'],['container','目标容器']])node('option',title,input).value=value;
      input.value=draft[key];input.oninput=input.onchange=()=>draft[key]=input.value;
    }
    const label=node('label',null,content),consent=node('input',null,label);consent.type='checkbox';consent.dataset.containerAgentConsent='true';label.append(document.createTextNode('明确允许本次 Agent 容器任务；期望条件不构成任意世界权限。'));
    const start=node('button','启动 Agent 自身容器任务',content);start.dataset.containerAgentStart=job.packageId;
    start.onclick=async()=>{start.disabled=true;try{if(!consent.checked||!draft.goal.trim())throw new Error('CONTAINER_CONSENT_AND_GOAL_REQUIRED');const count=Number(draft.count);if(!Number.isInteger(count)||count<1||count>8192)throw new Error('CONTAINER_EXPECTED_COUNT');
      ...
      node('p','已提交实际页面加载，任务必须经过 Native Menu 验证后才算完成。',content);
  ...
  function refreshRepair(){if(!repairForm)return;const current=jobFor(repairForm.source);const matched=repairDrafts.matches(current);
    if(repairForm.taskId&&repairForm.receipt){const result=jobs.find(j=>j.taskId===repairForm.taskId);if(result)repairForm.receipt.textContent="修复 Task "+result.taskId+" · "+result.state+" · "+(result.errorCode||'仅发布未启用；请审查候选内容。');}
    repairForm.start.disabled=repairForm.pending||!matched||!repairForm.consent.checked||!repairForm.prompt.value.trim();
    repairForm.notice.textContent=matched?'这是新的模型请求，可能再次计费；不会重放原 operation，也不会批准原生代码。':'原失败记录已变化或当前无权修复；未派发新请求。';
  }
  function openRepair(job){try{
    const value=repairDrafts.begin(job),content=windowFor('runtime-generation-repair','修复失败生成 · 新请求');content.replaceChildren();
    node('p',`原操作 ${job.operationId} · ${job.purpose} · 记录 r${job.jobRevision}`,content).className='muted';
    node('p',`原始输出 SHA-256: ${job.rawOutputSha256}`,content).className='repair-source-hash';
    node('p','原失败产物保留不变；修复将创建新的 Task 和 Package，严格校验后仅签名入库。未知结果不自动重试。',content).className='muted';
    const label=node('label','本次明确修复要求',content);label.htmlFor='generation-repair-prompt';
    const prompt=node('textarea',null,content);prompt.id='generation-repair-prompt';prompt.maxLength=8192;prompt.value=value.prompt;prompt.placeholder='指出要修复的行为与应保留的目标；不要填写 API Key';
    const consentLabel=node('label',null,content),consent=node('input',null,consentLabel);consent.type='checkbox';consent.id='generation-repair-consent';consentLabel.append(document.createTextNode('我明确提交这一次新的模型修复请求，并知悉可能再次计费。'));
    const notice=node('p',null,content);notice.className='muted';notice.setAttribute('role','status');
    const start=node('button','提交本次修复请求',content);start.id='generation-repair-submit';
    ...
    start.onclick=async()=>{form.pending=true;refreshRepair();try{
      ...
      const status=node('p',`修复 Task ${receipt.values.taskId} · ${receipt.values.state}；请查看生成列表终态，受理不等于完成。`,content);status.id='generation-repair-receipt';status.dataset.taskId=receipt.values.taskId;form.taskId=receipt.values.taskId;form.receipt=status;
  ...
  function renderJobs() {
    ...
    const previous=node('button','上一页绑定视图',navigation);previous.disabled=paging.page<=0;previous.onclick=()=>send('scoreViewPage',{page:paging.page-1}).catch(report);
    node('span',`绑定视图 ${paging.page+1}/${paging.pages} · ${paging.count} 个`,navigation);
    const next=node('button','下一页绑定视图',navigation);next.disabled=paging.page+1>=paging.pages;next.onclick=()=>send('scoreViewPage',{page:paging.page+1}).catch(report);
    if(focused){node('p','当前定位历史操作快照；不自动重放。Native 仍会复核当前版本、权限和确认。',list).className='muted';node('button','刷新此操作',list).onclick=()=>openHistorical(focused.category,focused.job.operationId);node('button','返回近期操作',list).onclick=()=>{focused=null;renderJobs();};}
    for (const job of focused?(focused.category==='GENERATION'?[jobFor(focused.job.operationId)]:[]):jobs) {
      ...
      if(job.repairOf)node('p',`修复来源 operation: ${job.repairOf}`,card).className='muted';
      if(job.state==='FAILED'&&job.repairable===true){const repair=node('button','修复保留的失败产物…',card);repair.dataset.repairGeneration=job.operationId;repair.onclick=()=>openRepair(job);}
      if (job.state === 'GENERATING') node('button', '取消生成', card).onclick = async event => {
      ...
      if(job.state==='PUBLISHED'&&(headFor(job.packageId)?.clientScriptAvailable||headFor(job.packageId)?.clientJavaAvailable)&&openClientScript)node('button','本机 CLIENT 原生代码下载 / 执行',card).onclick=()=>openClientScript(job.packageId);
      if(job.state==='PUBLISHED'&&headFor(job.packageId)?.resourcePackAvailable&&openResourcePack){node('button','本机资源包下载 / 启用',card).onclick=()=>openResourcePack(job.packageId);continue;}
      if(job.state==='PUBLISHED'&&headFor(job.packageId)?.dataPackAvailable&&openDataPack){node('button','数据包安装 / 停用',card).onclick=()=>openDataPack(job.packageId);continue;}
      ...
      if (job.state === 'PUBLISHED') node('button', '打开独立预览', card).onclick = async event => {
        ...
        try { const r = await send('packageAction', { action: 'preview', packageId: job.packageId, packageRevision: currentRevision });
          ...
          node('p', '已校验并提交页面加载；预览不产生世界写入。', card);
          const detail = node('details', null, card); node('summary', '下载与校验回执', detail);
          node('pre', JSON.stringify(r.values, null, 2), detail).className = 'preview-receipt';
        } catch (e) { report(e); } finally { button.disabled = false; }
      };
      if (job.state === 'PUBLISHED') {
        if(headFor(job.packageId)?.containerAvailable){
          const agentOpen=node('button','让 Agent 操作自身容器',card);agentOpen.dataset.containerAgentPackage=job.packageId;agentOpen.onclick=()=>agentContainer(job,currentRevision);
          const label=node('label',null,card);const consent=node('input',null,label);consent.type='checkbox';consent.dataset.containerConsent=job.packageId;label.append(document.createTextNode('允许此包通过原生交互打开准星容器，并读写我的槽位；隐藏/关闭时归还光标物品'));
          const open=node('button','打开准星容器网页',card);open.dataset.containerOpen=job.packageId;
        ...
        node('p',"生成版本 r"+job.packageRevision+" · 当前 r"+currentRevision,card).className='muted';
        const edit=node('button','修改包网页',card);edit.dataset.patchPackage=job.packageId;edit.onclick=()=>openPatch(job,currentRevision);
        for (const view of views.filter(v => v.packageId === job.packageId)) {
          ...
          if(headFor(job.packageId)?.hudAvailable===true){
            const hud=node('button',`打开只读 HUD ${view.id.slice(0,8)}`,card);hud.dataset.hudViewId=view.id;
            hud.title='真实计分源只读展示；关闭对话后继续刷新，不获得网页写权限';
          ...
          const open = node('button', `打开绑定视图 ${view.id.slice(0, 8)}`, card);
          ...
          const restore=node('button',`恢复本地草稿 ${view.id.slice(0,8)}`,card);
        ...
        if (sources.length) {
          const label=node('label','绑定已有计分目标（只创建视图，不修改分数）',card);
          const source=node('select',null,label);source.setAttribute('aria-label','已有计分目标');
          node('option','请选择已有目标',source).value='';
          ...
          const bind=node('button','创建独立绑定视图',card);
          bind.onclick=async()=>{
            bind.disabled=true;
            try{
  ...
  function openPatch(job,revision){
    ...
    const content=windowFor('runtime-ui-patch','修改现有包网页');content.replaceChildren();
    node('p',`基于 r${revision} 的独立候选，不立即覆盖当前页面；取消不重放模型。`,content).className='muted';
    const label=node('label','修改要求',content);label.htmlFor='ui-patch-prompt';
    ...
    const submit=node('button','生成改版候选',content);submit.id='ui-patch-submit';
    submit.onclick=async()=>{submit.disabled=true;try{patchDraft.edit(input.value);const args=patchDraft.request(()=>crypto.randomUUID());persist();const r=await send('packageAction',{action:'patchSubmit',...args});if(r.code!=='ACCEPTED')throw new Error(r.values?.errorCode||r.code);node('p',`服务端 ${r.values.state} · ${r.values.taskId}`,content);}catch(e){report(e);}finally{submit.disabled=false;}};
  }
  function renderPatches(parent,items=patchJobs){
    if(!items.length)return;node('h3','网页改版历史',parent);
    for(const job of items){
      ...
      if(job.rebuilt)node('p','已从本地接收输出重建 · 未再调用模型 · 原 FAILED 记录保留',card).className='muted';
      if(job.state==='FAILED'&&job.rebuildAllowed){
        ...
        const details=node('details',null,card);details.open=draft.open===true;details.ontoggle=()=>draft.open=details.open;node('summary','Operator：从已接收输出重建（不调用模型）',details);
        node('p','仅当确认此内容库 SHA 属于本失败任务时使用。原始归属缺失会明确记录为 Operator 人工确认；不会覆盖原失败，也不会自动应用候选。',details).className='muted';
        const input=node('input',null,details);input.setAttribute('aria-label','已接收输出 SHA-256');input.maxLength=64;input.value=draft.hash;input.dataset.patchRebuildSha=job.operationId;
        const label=node('label',null,details),confirm=node('input',null,label);confirm.type='checkbox';confirm.checked=draft.confirmed;confirm.dataset.patchRebuildConsent=job.operationId;label.append(document.createTextNode('我已核对该 SHA 的来源，明确允许只读解析并重建此 UI 候选'));
        ...
        const rebuild=node('button','重建已接收的 UI 输出',details);rebuild.dataset.patchRebuild=job.operationId;rebuild.disabled=draft.busy;
        rebuild.onclick=async()=>{rebuild.disabled=true;draft.busy=true;try{const request=uiPatchRebuildRequest(job,draft.hash,draft.confirmed);const r=await send('packageAction',request);if(r.code!=='APPLIED')throw new Error(r.values?.errorCode||r.code);draft.status=`${r.values.state} · 本地重建，无 Provider 调用`;status.textContent=draft.status;}catch(e){draft.status=e.message;status.textContent=e.message;report(e);}finally{draft.busy=false;rebuild.disabled=false;}};
      }
      ...
      if(job.state==='READY'){
        const available=views.filter(v=>v.packageId===job.packageId);const target=node('select',null,card);target.setAttribute('aria-label','候选预览绑定视图');node('option','不绑定业务（纯预览）',target).value='';
        ...
        const preview=node('button','只读预览候选',card);preview.dataset.patchAction='patchPreview';preview.onclick=async()=>{preview.disabled=true;try{const r=await send('packageAction',{action:'patchPreview',operationId:job.operationId,packageId:job.packageId,packageRevision:job.candidateRevision,targetViewId:target.value});if(r.code!=='ACCEPTED')throw new Error(r.values?.errorCode||r.code);}catch(e){report(e);}finally{preview.disabled=false;}};
        command('应用并切换窗口（草稿只读恢复）','patchApplySwap');
        command('仅应用包（旧页面保留草稿）','patchApply');
      }
      if(['PENDING','READY'].includes(job.state))command('取消改版','patchCancel');
      if(job.state==='APPLIED'){command('回退并切换窗口（不回退业务数据）','patchUndoSwap');command('仅回退包','patchRollback');}
    }
  }
  function open() {
    const content = windowFor('runtime-generation', '内容与网页生成任务');
    if (content.querySelector('#generation-jobs')) { renderJobs(); return; } content.replaceChildren();
    const managed=node('button','管理已有世界实例',content);managed.id='world-instances-open';managed.onclick=()=>worldContents.manage();
    node('p', '使用服务器配置的 Provider 生成独立 RuntimePackage，消耗模型额度。产物签名入库但不启用玩法。', content).className = 'muted';
    const label = node('label', '执行 Agent', content); label.htmlFor = 'generation-agent';
    const agent = node('select', null, content); agent.id = 'generation-agent'; fillAgents();
    node('label','生成范围',content);const purpose=node('select',null,content);purpose.id='generation-purpose';node('option','独立网页包',purpose).value='UI_PACKAGE';node('option','独立世界内容（原生代码需另行启用）',purpose).value='WORLD_CONTENT';purpose.value=draft.purpose;
    const promptLabel = node('label', '内容或网页需求', content); promptLabel.htmlFor = 'generation-prompt';
    const prompt = node('textarea', null, content); prompt.id = 'generation-prompt'; prompt.maxLength = 8192; prompt.value = draft.prompt;
    prompt.placeholder = '描述独立窗口、表单、交互和布局；不要填写 API Key';
    agent.onchange = prompt.oninput = purpose.onchange = () => { syncDraft(); persist(); };
    const start = node('button', '生成所选范围的包', content); start.id = 'generation-submit';
    start.onclick = async () => {
      start.disabled = true;
      try {
        ...
        node('p', `服务端任务 ${receipt.values.taskId}：${receipt.values.state}。关闭窗口不会取消任务。`, content);
      } catch (e) { report(e); } finally { start.disabled = false; }
    };
    const reset = node('button', '作为新任务提交', content); reset.onclick = () => { draft.operationId = null; persist(); start.click(); };
    const plan=node('button','由 Agent 规划网页任务',content);plan.id='ui-task-submit';
    plan.onclick=async()=>{plan.disabled=true;try{
      if([...prompt.value].length>256)throw new Error('网页任务入口最多 256 字；详细直接生成仍可使用原按钮。');
      if(purpose.value!=='UI_PACKAGE')throw new Error('世界内容请使用直接生成入口；通用 Agent 世界包工具尚未接通。');
      ...
      const result=node('p',`Agent 任务 ${receipt.values.taskId} · ${receipt.values.state}；工具必须等待实际产物，改版候选仍需明确应用。`,content);result.id='ui-task-receipt';result.dataset.taskId=receipt.values.taskId;
    }catch(error){report(error);}finally{plan.disabled=false;}};
    const newPlan=node('button','新建 Agent 网页任务',content);newPlan.onclick=()=>{taskDraft.operationId=null;persist();plan.click();};
    node('p', '重试保留 operationId，不重复创建；“作为新任务提交”会明确创建新任务并调用模型。取消不撤回已产生的模型费用。', content).className = 'muted';
  ...
  async function openPackageTools(packageId){
    const epoch=contextEpoch;try{
      ...
      const head=JSON.parse(receipt.values.state),content=windowFor('runtime-package-tools','当前自有包操作');content.replaceChildren();
      node('strong',head.name+' · r'+head.revision,content);node('p','来源 '+head.origin+'；这里只使用当前真实包，不创建伪造生成任务。Native 会再次检查版本与权限。',content).className='muted';
      const label=node('label','执行 Agent（权限以服务端复核为准）',content),agent=node('select',null,label);node('option','选择 Agent',agent).value='';for(const a of agents)node('option',a.name,agent).value=a.id;
      ...
      function render(){body.replaceChildren();const descriptor={packageId:head.packageId,agentId:selected,packageRevision:head.revision};
        if(head.uiAvailable){const preview=node('button','打开当前独立预览',body);preview.onclick=async()=>{preview.disabled=true;try{const r=await send('packageAction',{action:'preview',packageId:head.packageId,packageRevision:head.revision});if(r.code!=='ACCEPTED')throw new Error(r.values?.errorCode||r.code);}catch(e){report(e);}finally{preview.disabled=false;}};
          const edit=node('button','修改包内网页…',body);edit.disabled=!selected;edit.onclick=()=>openPatch(descriptor,head.revision);}
        if(head.resourcePackAvailable&&openResourcePack)node('button','本机资源包下载 / 启用',body).onclick=()=>openResourcePack(packageId);
        if((head.clientScriptAvailable||head.clientJavaAvailable)&&openClientScript)node('button','本机 CLIENT 原生代码下载 / 执行',body).onclick=()=>openClientScript(packageId);
        if(head.dataPackAvailable&&openDataPack)node('button','数据包安装 / 停用',body).onclick=()=>openDataPack(packageId);
        if(head.bootAvailable){descriptor.activationMode='BOOT_EXTENSION';if(selected)worldPatches.render(body,descriptor,head.revision);else node('p','选择 Agent 后可提出启动扩展同包改版；发布不会替换正在运行的 Mod。',body).className='muted';}
        if(head.worldAvailable){if(selected){worldContents.render(body,descriptor,head.revision);worldPatches.render(body,descriptor,head.revision);}else node('p','选择 Agent 后可打开原世界内容审批与改版入口。',body).className='muted';}
        if(head.containerAvailable){const agentButton=node('button','Agent 自身容器操作…',body);agentButton.disabled=!selected;agentButton.onclick=()=>agentContainer(descriptor,head.revision);
          const line=node('label',null,body),consent=node('input',null,line);consent.type='checkbox';line.append(document.createTextNode('允许当前包通过原生容器页面读写我的槽位；不是代理其它玩家的库存。'));
          const playerButton=node('button','打开准星容器网页',body);playerButton.onclick=async()=>{if(!consent.checked)return;playerButton.disabled=true;try{const r=await send('packageAction',{action:'containerOpen',packageId:head.packageId,packageRevision:head.revision,confirmed:true});if(r.code!=='ACCEPTED')throw new Error(r.values?.errorCode||r.code);}catch(e){report(e);}finally{consent.checked=false;playerButton.disabled=false;}};}
        for(const view of views.filter(v=>v.packageId===head.packageId)){worldBoards.render(body,descriptor,head.revision,view);
          for(const [title,action,available] of [['打开绑定视图','open',head.uiAvailable],['打开只读 HUD','hud',head.hudAvailable]])if(available){const b=node('button',title+' '+view.id.slice(0,8),body);b.onclick=async()=>{b.disabled=true;try{const r=await send('packageAction',{action,packageId:head.packageId,packageRevision:head.revision,targetViewId:view.id});if(r.code!=='ACCEPTED')throw new Error(r.values?.errorCode||r.code);}catch(e){report(e);}finally{b.disabled=false;}};}
        }
        if(sources.length&&head.uiAvailable){const line=node('label','绑定已有计分目标',body),source=node('select',null,line);node('option','选择已有目标',source).value='';for(const s of sources)node('option',s.reference,source).value=s.id;
          const bind=node('button','创建独立绑定视图',body);bind.onclick=async()=>{bind.disabled=true;try{const chosen=sources.find(s=>s.id===source.value);if(!chosen)throw new Error('BINDING_SOURCE_REQUIRED');bindings.select(head.packageId,chosen.id);const request=bindings.request(head.packageId,chosen.reference,()=>crypto.randomUUID());persist();const r=await send('packageAction',{action:'bind',packageId:head.packageId,packageRevision:head.revision,...request});if(r.code!=='APPLIED')throw new Error(r.values?.errorCode||r.code);}catch(e){report(e);}finally{bind.disabled=false;}};}
        node('p','绑定视图使用当前分页数据；可在内容生成页翻页，再刷新此面板。不会把未返回的视图当作不存在。',body).className='muted';
      }
      agent.onchange=()=>{selected=agent.value;render();};render();node('button','重读包版本与当前绑定数据',content).onclick=()=>openPackageTools(packageId);
  ...
  function session(value){
    ...
    for(const content of ownedWindows){content.replaceChildren();if(content.isConnected)node('p','连接上下文已变化，请从当前目录或历史重新打开操作。',content).className='muted';else ownedWindows.delete(content);}
